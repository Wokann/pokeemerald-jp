...
# Match the official agbcc newlib build: libc uses old_agbcc with -O2 -fno-builtin.
$(C_BUILDDIR)/libc.o: CC := ../pokeemerald/tools/agbcc/bin/old_agbcc
$(C_BUILDDIR)/libc.o: CFLAGS := -O2 -fno-builtin -ffunction-sections
$(C_BUILDDIR)/libc.o: src/libc.c
	@mkdir -p $(C_BUILDDIR)
	@set -o pipefail; { $(CPP) $(CPPFLAGS) -P -x c $< | $(CC) $(CFLAGS) -o - -; } > $(C_BUILDDIR)/libc_gen.s
	@awk '/^\.Lfe[0-9]+:/{print "\t.align\t2, 0"} {print}' $(C_BUILDDIR)/libc_gen.s | $(AS) $(ASFLAGS) -o $@ -
	@rm -f $(C_BUILDDIR)/libc_gen.s
