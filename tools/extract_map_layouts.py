...
Both legacy ``gMapLayout_*`` labels and canonical ``*_Layout`` labels are
accepted so that historical partial migration remains auditable. Canonical
labels without ROM-address comments resolve their physical address through the
checked-in local ``data/layouts/layouts.json`` order and the JP ROM table.
...
TABLE_PATH = OUT_BASE / "layouts_table.inc"
...
def load_layout_area_names():
    """Layout suffix -> canonical area directory name."""
    names = {}
    if LAYOUTS_JSON.exists():
...
def load_layout_names():
    """Return canonical layout symbol -> layout suffix mappings."""
    names = {}
    if LAYOUTS_JSON.exists():
        data = json.loads(LAYOUTS_JSON.read_text(encoding="utf-8"))
        for entry in data["layouts"]:
...
def load_layout_ids():
    """Return the layout IDs in the canonical table order."""
    if not LAYOUTS_JSON.exists():
        raise FileNotFoundError(f"missing layout metadata: {LAYOUTS_JSON}")
    data = json.loads(LAYOUTS_JSON.read_text(encoding="utf-8"))
    return [entry["id"] for entry in data["layouts"]]


def load_layout_addresses(rom):
    """Return canonical layout symbols mapped to their JP ROM struct addresses."""
    if len(rom) < TABLE_OFFSET + TABLE_SIZE:
        raise ValueError("baserom is too short for the MapLayout pointer table")
    data = json.loads(LAYOUTS_JSON.read_text(encoding="utf-8"))
    entries = data.get("layouts")
    if not isinstance(entries, list) or len(entries) != TABLE_SIZE // 4:
        raise ValueError("layout metadata count does not match the JP MapLayout table")
    pointers = struct.unpack_from(f"<{len(entries)}I", rom, TABLE_OFFSET)
    return {entry["name"]: pointer for entry, pointer in zip(entries, pointers)}


def build_layouts_table_text(entries, rom):
    """Emit the JP MapLayout table without inventing labels for raw entries."""
    entry_count = TABLE_SIZE // 4
    if len(rom) < TABLE_OFFSET + TABLE_SIZE:
        raise ValueError("baserom is too short for the MapLayout pointer table")

    layout_ids = load_layout_ids()
    if len(layout_ids) != entry_count:
        raise ValueError(
            f"layout metadata has {len(layout_ids)} entries, expected {entry_count}"
        )
    ...
    for entry in entries:
        ...
        if addr in symbols_by_addr:
            raise ValueError(f"duplicate structured MapLayout address: 0x{addr:08X}")
        symbols_by_addr[addr] = entry["symbol"]

    pointers = struct.unpack_from(f"<{entry_count}I", rom, TABLE_OFFSET)
    lines = [
        "@",
    ...
    def flush_raw_span(end):
        nonlocal raw_start, raw_spans
        if raw_start is None:
            return
        first_id = layout_ids[raw_start]
        last_id = layout_ids[end - 1]
        if raw_start + 1 == end:
            lines.append(f"\t@ {first_id} remains in its raw physical layout owner.")
        else:
            lines.append(
                f"\t@ {first_id} through {last_id} remain in raw physical layout owners."
            )
        lines.append(
            f'\t.incbin "baserom_jp.gba", 0x{TABLE_OFFSET + raw_start * 4:X}, '
            f"0x{(end - raw_start) * 4:X}"
        )
        raw_start = None
        raw_spans += 1

    for index, pointer in enumerate(pointers):
        symbol = symbols_by_addr.get(pointer)
        if symbol is None:
            if raw_start is None:
                raw_start = index
            raw_entries += 1
            continue
        flush_raw_span(index)
        lines.append(f"\t.4byte {symbol}")
        used_symbols.add(symbol)
    flush_raw_span(entry_count)

    unused_symbols = sorted(set(symbols_by_addr.values()) - used_symbols)
    if unused_symbols:
        raise ValueError(
            "structured MapLayouts not referenced by gMapLayouts: "
            + ", ".join(unused_symbols)
        )

    return "\n".join(lines) + "\n", len(used_symbols), raw_entries, raw_spans
...
def parse_layouts(lines, rom, canonical_layout_names, canonical_layout_addresses):
    entries = []
    raw_layouts = []
    for i, line in enumerate(lines):
        ...
        elif legacy_name is None:
            try:
                addr = canonical_layout_addresses[symbol]
            except KeyError as error:
                raise ValueError(f"no JP ROM address for canonical symbol: {symbol}") from error
        else:
            raise ValueError(f"legacy layout label lacks a ROM address: {symbol}")
        defs = {}
...
def main():
    ...
    mode.add_argument("--check", action="store_true", help="verify only, no writes")
    mode.add_argument(
        "--write-table",
        action="store_true",
        help="write data/layouts/layouts_table.inc without extracting layout resources",
    )
    mode.add_argument(
        "--check-table",
        action="store_true",
        help="verify data/layouts/layouts_table.inc matches the current JP ROM and layouts",
    )
    args = ap.parse_args()
    ...
    area_names = load_layout_area_names()
    canonical_layout_names = load_layout_names()
    canonical_layout_addresses = load_layout_addresses(rom)
    entries, raw_layouts = parse_layouts(
        lines,
        rom,
        canonical_layout_names,
        canonical_layout_addresses,
    )
    ...
    if args.write_table or args.check_table:
        table_text, structured, raw_entries, raw_spans = build_layouts_table_text(entries, rom)
        if args.write_table:
        ...
        elif not TABLE_PATH.exists() or TABLE_PATH.read_text(encoding="utf-8") != table_text:
            print(f"table does not match: {TABLE_PATH}")
            sys.exit(1)
        print(
            f"layouts table: {structured + raw_entries} entries "
            f"({structured} structured, {raw_entries} raw entries in {raw_spans} spans)"
        )
        return
