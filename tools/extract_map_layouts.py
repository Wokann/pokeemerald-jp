...
def load_us_layout_ids():
    """Return the layout IDs in the canonical table order."""
    if not US_LAYOUTS_JSON.exists():
        raise FileNotFoundError(f"missing US layout metadata: {US_LAYOUTS_JSON}")
    us = json.loads(US_LAYOUTS_JSON.read_text(encoding="utf-8"))
    return [entry["id"] for entry in us["layouts"]]
...
def build_layouts_table_text(entries, rom):
    ...
    layout_ids = load_us_layout_ids()
    if len(layout_ids) != entry_count:
        raise ValueError(
            f"US layout metadata has {len(layout_ids)} entries, expected {entry_count}"
        )
    ...
    lines = [
        "@",
        "@ This file is generated by tools/extract_map_layouts.py --write-table.",
        "@ Unmigrated MapLayout entries remain byte-exact pointer gaps until their",
        "@ physical layout owners have been extracted into data/layouts/.",
        "@",
        "",
        "\t.align 2",
        "gMapLayouts:: @ 0x0845A1F4",
    ]
    used_symbols = set()
    raw_entries = 0
    raw_spans = 0
