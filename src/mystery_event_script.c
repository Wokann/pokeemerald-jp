...
extern struct ScriptContext sMysteryEventScriptContext;
...
void InitMysteryEventScript(struct ScriptContext *ctx, u8 *script)
{
...
int CalcRecordMixingGiftChecksum(void);
...
bool8 MEScrCmd_givepokemon(struct ScriptContext *ctx)
{
    ...
    else
    {
        ...
        if (species != SPECIES_EGG)
        {
            u16 pokedexNum = HoennToNationalOrder(species);
