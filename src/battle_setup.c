...
void TryUpdateGymLeaderRematchFromWild(void);
void TryUpdateGymLeaderRematchFromTrainer(void);
...
// JP: these live in the JP EWRAM section (0x02038868-0x0203889C);
// wired via ld_script_jp.txt
extern EWRAM_DATA u16 sTrainerBattleMode;
extern EWRAM_DATA u16 gTrainerBattleOpponent_A;
extern EWRAM_DATA u16 gTrainerBattleOpponent_B;
extern EWRAM_DATA u16 gPartnerTrainerId;
extern EWRAM_DATA u16 sTrainerObjectEventLocalId;
extern EWRAM_DATA u8 *sTrainerAIntroSpeech;
extern EWRAM_DATA u8 *sTrainerBIntroSpeech;
extern EWRAM_DATA u8 *sTrainerADefeatSpeech;
extern EWRAM_DATA u8 *sTrainerBDefeatSpeech;
extern EWRAM_DATA u8 *sTrainerVictorySpeech;
extern EWRAM_DATA u8 *sTrainerCannotBattleSpeech;
extern EWRAM_DATA u8 *sTrainerBattleEndScript;
extern EWRAM_DATA u8 *sTrainerABattleScriptRetAddr;
extern EWRAM_DATA u8 *sTrainerBBattleScriptRetAddr;
...
static void Task_BattleStart(u8 taskId)
{
    ...
    {
    case 0:
        if (!sub_080FB730()) // JP: uses sub_080FB730 instead of FldEffPoison_IsActive
        {
...
void DoBattlePyramidTrainerHillBattle(void)
{
    if (CurrentBattlePyramidLocation() != PYRAMID_LOCATION_NONE)
...
{
    // JP: GetTrainerAFlag reads the opponent id stored at 0x0203886C
    return TRAINER_FLAGS_START + gTrainerBattleOpponent_B;
}

static u16 GetTrainerBFlag(void)
{
    // JP: GetTrainerBFlag reads the opponent id stored at 0x0203886A
...
bool8 GetTrainerFlag(void)
{
    if (CurrentBattlePyramidLocation() != PYRAMID_LOCATION_NONE)
        ...
        return FlagGet(GetTrainerBFlag());
}
...
{
    FlagClear(TRAINER_FLAGS_START + trainerId); // JP: Set calls FlagClear
}

void ClearTrainerFlag(u16 trainerId)
{
    FlagSet(TRAINER_FLAGS_START + trainerId); // JP: Clear calls FlagSet
}
