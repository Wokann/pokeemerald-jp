...
bool8 ReadFlashSector(u8, struct SaveSector *);
u8 GetSaveValidStatus(const struct SaveSectorLocation *);
u8 CopySaveSlotData(u16, struct SaveSectorLocation *);
u8 TryWriteSector(u8, u8 *);
u8 HandleWriteSector(u16, const struct SaveSectorLocation *);
u8 HandleReplaceSector(u16, const struct SaveSectorLocation *);

// JP ROM-resident RAM (bound in sym_iwram_jp.txt / sym_ewram_jp.txt).
extern IWRAM_DATA u16 gLastWrittenSector;
extern IWRAM_DATA u32 gLastSaveCounter;
extern IWRAM_DATA u16 gLastKnownGoodSector;
extern IWRAM_DATA u32 gDamagedSaveSectors;
extern IWRAM_DATA u32 gSaveCounter;
extern IWRAM_DATA struct SaveSector *gReadWriteSector;
extern IWRAM_DATA u16 gIncrementalSectorId;
extern IWRAM_DATA u16 gSaveFileStatus;
extern IWRAM_DATA MainCallback gGameContinueCallback;
extern IWRAM_DATA struct SaveSectorLocation gRamSaveSectorLocations[NUM_SECTORS_PER_SLOT];
...
u8 WriteSaveSectorOrSlot(u16 sectorId, const struct SaveSectorLocation *locations)
{
    u32 status;
    u16 i;
...
u8 HandleWriteSector(u16 sectorId, const struct SaveSectorLocation *locations)
{
    u16 i;
    u16 sector;
    u8 *data;
...
u8 HandleWriteSectorNBytes(u8 sectorId, u8 *data, u16 size)
{
    u16 i;
...
u32 RestoreSaveBackupVarsAndIncrement(const struct SaveSectorLocation *locations)
{
    gReadWriteSector = &gSaveDataBuffer;
...
u32 RestoreSaveBackupVars(const struct SaveSectorLocation *locations)
{
    gReadWriteSector = &gSaveDataBuffer;
    gLastKnownGoodSector = gLastWrittenSector;
    gLastSaveCounter = gSaveCounter;
...
u8 HandleWriteIncrementalSector(u16 numSectors, const struct SaveSectorLocation *locations)
{
    u8 status;
...
u8 HandleReplaceSectorAndVerify(u16 sectorId, const struct SaveSectorLocation *locations)
{
...
u8 HandleReplaceSector(u16 sectorId, const struct SaveSectorLocation *locations)
{
    u16 i;
    u16 sector;
    u8 *data;
...
u8 CopySectorSignatureByte(u16 sectorId, const struct SaveSectorLocation *locations)
{
    // Adjust sector id for current save slot
...
u8 WriteSectorSignatureByte(u16 sectorId, const struct SaveSectorLocation *locations)
{
    // Adjust sector id for current save slot
    u16 sector = sectorId + gLastWrittenSector - 1;
    sector %= NUM_SECTORS_PER_SLOT;
...
u8 TryLoadSaveSlot(u16 sectorId, struct SaveSectorLocation *locations)
{
    u8 status;
    gReadWriteSector = &gSaveDataBuffer;
    if (sectorId != FULL_SAVE_SLOT)
...
u8 CopySaveSlotData(u16 sectorId, struct SaveSectorLocation *locations)
{
    u16 i;
    u16 checksum;
...
u8 GetSaveValidStatus(const struct SaveSectorLocation *locations)
{
    u16 i;
    u16 checksum;
...
u8 TryLoadSaveSector(u8 sectorId, u8 *data, u16 size)
{
    u16 i;
    struct SaveSector *sector = &gSaveDataBuffer;
...
bool8 ReadFlashSector(u8 sectorId, struct SaveSector *sector)
{
...
u8 HandleSavingData(u8 saveType)
{
    ...
    case SAVE_EREADER: // Dummied, now duplicate of SAVE_LINK
        ...
        // JP writes each of the five SaveBlock2/SaveBlock1 sectors in turn
        // with the ordinary sector writer (no erase/signature split).
        CopyPartyAndObjectsToSave();
