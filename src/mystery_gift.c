...
extern void ClearMysteryGiftVars(void);
extern void InitQuestionnaireWords(void);
extern const char sMeventAssertFile[]; // "mevent.c"
extern const char sMeventAssertExpr0[]; // "0"
...
void ClearSavedWonderNewsMetadata(void);
...
bool32 SaveWonderNews(const struct WonderNews *news)
{
    ...
    ClearSavedWonderNews();
    memcpy(&gSaveBlock1Ptr->mysteryGift.news, news, sizeof(*news));
    gSaveBlock1Ptr->mysteryGift.newsCrc = (u16)CALC_CRC(gSaveBlock1Ptr->mysteryGift.news);
    return TRUE;
}

bool32 ValidateSavedWonderNews(void)
{
    if ((u16)CALC_CRC(gSaveBlock1Ptr->mysteryGift.news) != gSaveBlock1Ptr->mysteryGift.newsCrc)
        return FALSE;
...
{
    u32 fill = 0;
    CpuSet(&fill, GetSavedWonderNews(), 0x05000038);
...
bool32 SaveWonderCard(const struct WonderCard *card)
{
    ...
    gSaveBlock1Ptr->mysteryGift.cardCrc = (u16)CALC_CRC(gSaveBlock1Ptr->mysteryGift.card);
...
{
    if (gSaveBlock1Ptr->mysteryGift.cardCrc != (u16)CALC_CRC(gSaveBlock1Ptr->mysteryGift.card))
        return FALSE;
...
{
    u32 fill = 0;
    CpuSet(&fill, &gSaveBlock1Ptr->mysteryGift.card, 0x05000029);
...
{
    u32 fill = 0;
    CpuSet(&fill, GetSavedWonderCardMetadata(), 0x05000009);
...
bool32 IsWonderCardFlagIDInValidRange(u16 flagId)
{
...
static int GetNumStampsInMetadata(const struct WonderCardMetadata *data, int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        if (data->stampData[STAMP_ID][i] != 0 && data->stampData[STAMP_SPECIES][i] != 0)
            numStamps++;
    }
    return numStamps;
}
...
static bool32 ValidateStamp(const u16 *stamp)
{
    if (stamp[STAMP_ID] == 0)
        return FALSE;
    if (stamp[STAMP_SPECIES] == 0)
        return FALSE;
    if (stamp[STAMP_SPECIES] > SPECIES_CHIMECHO) // JP last species (US uses SPECIES_DEOXYS)
        return FALSE;
    return TRUE;
}
...
u16 MysteryGift_GetCardStatFromLinkData(const struct MysteryGiftLinkGameData *data, u32 stat)
{
    ...
    default:
        AGBAssert(sMeventAssertFile, 0x34F, sMeventAssertExpr0, 1);
        return 0;
    }
}
...
static void IncrementCardStat(u32 statType)
{
    ...
    if (card->type == CARD_TYPE_LINK_STAT)
    {
        ...
        if (stat == NULL)
        {
            AGBAssert(sMeventAssertFile, 0x37A, sMeventAssertExpr0, 1);
        }
...
u16 MysteryGift_GetCardStat(u32 stat)
{
    ...
    AGBAssert(sMeventAssertFile, 0x3A7, sMeventAssertExpr0, 1);
    return 0;
}
...
void MysteryGift_TryIncrementStat(u32 stat, u32 trainerId)
{
    if (sStatsEnabled)
    {
        ...
        case CARD_STAT_BATTLES_LOST:
            ...
            break;
        default:
            AGBAssert(sMeventAssertFile, 0x3F2, sMeventAssertExpr0, 1);
            break;
        }
    }
}
...
{
    u32 fill = 0;
    CpuSet(&fill, gSaveBlock1Ptr->mysteryGift.trainerIds, 0x0500000A);
}
