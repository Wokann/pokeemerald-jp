...
void PrintMatchCallFieldNames(struct PokenavList *, u32);
...
static u32 LoopedTask_PrintListItems(s32 state)
{
    ...
    case 0:
        ...
        AddTextPrinterParameterized(list->listWindow.windowId, 1, list->itemTextBuffer, 0, (row << 4) + 2, TEXT_SKIP_DRAW, NULL);
...
static u32 LoopedTask_MoveListWindow(s32 state)
{
    ...
    case 1:
        ...
        newY = ChangeBgY(list->listWindow.bg, 0x800, list->bgMoveType);
...
static void PrintCheckPageTrainerName(struct PokenavListWindowState *state, struct PokenavList *list)
{
    ...
    memcpy(colors, sPokenavCheckPageColors, 3);
    ...
    AddTextPrinterParameterized3(list->listWindow.windowId, 1, 0, (list->listWindow.unkA * 16) + 2, colors, TEXT_SKIP_DRAW, list->itemTextBuffer);
...
static void PrintMatchCallListTrainerName(struct PokenavListWindowState *state, struct PokenavList *list)
{
    ...
    AddTextPrinterParameterized(list->listWindow.windowId, 1, list->itemTextBuffer, 0, list->listWindow.unkA * 16 + 2, TEXT_SKIP_DRAW, NULL);
    SetListMarginTile(&list->listWindow, FALSE);
    CopyWindowToVram(list->listWindow.windowId, COPYWIN_FULL);
}

// JP copies the field-name pointer table and text colors from ROM data at
...
static void PrintMatchCallFlavorText(struct PokenavListWindowState *windowState, struct PokenavList *list, u32 checkPageEntry)
{
    ...
    if (str != NULL)
        ...
        StringFill(list->itemTextBuffer, 0, list->listWindow.width);

    if (str != NULL)
    {
        AddTextPrinterParameterized(list->listWindow.windowId, 1, list->itemTextBuffer, 0, (r7 << 4) + 2, TEXT_SKIP_DRAW, NULL);
        CopyWindowRectToVram(list->listWindow.windowId, COPYWIN_GFX, 0, r7 * 2, list->listWindow.width, 2);
    }
}
...
static void CreateListArrowSprites(const struct PokenavListWindowState *windowState, struct PokenavList *list)
{
    ...
    spriteId = CreateSprite(&sSpriteTemplate_RightArrow, (s16)(list->listWindow.x * 8 - 4), (list->listWindow.y + 1) * 8, 7);
