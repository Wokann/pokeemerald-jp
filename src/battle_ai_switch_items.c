...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    {
        battlerPosition = GetBattlerPosition(gActiveBattler) ^ BIT_SIDE;
        opposingBattler = GetBattlerAtPosition(battlerPosition);
        battlerIn1 = gActiveBattler;
        battlerIn2 = gActiveBattler;
    }
    ...
    if ((s8)GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER)
        party = gPlayerParty;
    else
        party = gEnemyParty;
...
bool8 ShouldUseItem(void)
{
    ...
    {
        battlerPosition = GetBattlerPosition(gActiveBattler);
        if (battlerPosition == B_POSITION_PLAYER_RIGHT)
            return FALSE;
    }
