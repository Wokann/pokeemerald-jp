...
extern EWRAM_DATA u16 ALIGNED(4) sBackupMapData[MAX_MAP_DATA_SIZE];
extern EWRAM_DATA struct MapHeader gMapHeader;
extern EWRAM_DATA struct Camera gCamera;
extern EWRAM_DATA struct ConnectionFlags sMapConnectionFlags;
extern EWRAM_DATA u32 sFiller;
extern COMMON_DATA struct BackupMapLayout gBackupMapLayout;
...
bool8 IsPosInIncomingConnectingMap(u8 direction, s32 x, s32 y, const struct MapConnection *connection);
bool8 IsCoordInIncomingConnectingMap(s32 coord, s32 srcMax, s32 destMax, s32 offset);
...
void FillConnection(s32 x, s32 y, const struct MapHeader *connectedMapHeader, s32 x2, s32 y2, s32 width, s32 height)
{
    s32 i;
    const u16 *src;
    u16 *dest;
...
void MoveMapViewToBackup(u8 direction)
{
    s32 width;
    s32 x0, y0;
    s32 x2, y2;
...
void SetPositionFromConnection(const struct MapConnection *connection, s32 direction, s32 x, s32 y)
{
...
s32 IsCoordInConnectingMap(s32 coord, s32 max)
{
...
s32 IsPosInConnectingMap(const struct MapConnection *connection, s32 x, s32 y)
{
    struct MapHeader const *mapHeader;
    mapHeader = GetMapHeaderFromConnection(connection);
    switch (connection->direction)
...
void CopyTilesetToVram(struct Tileset const *tileset, u16 numTiles, u16 offset)
{
    if (tileset)
    {
...
void CopyTilesetToVramUsingHeap(struct Tileset const *tileset, u16 numTiles, u16 offset)
{
    if (tileset)
    {
        if (!tileset->isCompressed)
...
void ApplyGlobalTintToPaletteEntries(u16 offset, u16 size)
{
...
void LoadPrimaryTilesetPalette(struct MapLayout const *mapLayout)
{
