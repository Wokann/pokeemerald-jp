...
// JP: these tables live in the JP data section; wired via ld_script_jp.txt
extern const s8 sTruckCamera_HorizontalTable[19];
extern const u8 sSSTidalSailEastMovementScript[2];
extern const u8 sSSTidalSailWestMovementScript[2];
...
{
    // JP: uses AddPseudoEventObject + GetJumpInPlaceMovementAction instead of
    // the US CreateObjectGraphicsSprite + GetFaceDirectionAnimNum pair.
    ...
    if (VarGet(VAR_SS_TIDAL_STATE) == SS_TIDAL_DEPART_SLATEPORT)
        StartSpriteAnim(&gSprites[spriteId], GetJumpInPlaceMovementAction(4));
    else
        StartSpriteAnim(&gSprites[spriteId], GetJumpInPlaceMovementAction(3));
}
