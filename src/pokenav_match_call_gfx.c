...
// JP-only alias: the `bx r1` at the end of IsMatchCallLoopedTaskActive is
// also labeled sub_081CAA80 (0x081CAA80) in the JP ROM.  No separate code.
__asm__(".set sub_081CAA80, IsMatchCallLoopedTaskActive + 0x10");
...
{
    PrintMatchCallInfoNumber(windowId, gText_NumberRegistered, 0);
}
...
{
    PrintMatchCallInfoNumber(windowId, gText_NumberOfBattles, 2);
}
...
// JP merges the label/number printers into one centered printer with
// FONT_NORMAL and y = top*16 + 2 (US used FONT_NARROW, x=2 and +1).
static void PrintMatchCallInfoNumber(u16 windowId, const u8 *str, int top)
{
    int x = 0x54 - GetStringWidth(FONT_NORMAL, str, -1);
    int y = top * 16 + 2;
...
static void PrintMatchCallLocation(struct Pokenav_MatchCallGfx *gfx, int delta)
{
    ...
    x = 0x54 - GetStringWidth(FONT_NORMAL, mapName, -1);
    FillWindowPixelBuffer(gfx->locWindowId, PIXEL_FILL(1));
    AddTextPrinterParameterized(gfx->locWindowId, FONT_NORMAL, mapName, x, 2, 0, NULL);
}
...
static void PrintMatchCallSelectionOptions(struct Pokenav_MatchCallGfx *gfx)
{
    ...
    for (i = 0; i < MATCH_CALL_OPTION_COUNT; i++)
    {
        ...
        AddTextPrinterParameterized(gfx->infoBoxWindowId, FONT_NORMAL, sMatchCallOptionTexts[optionText], 0x14, i * 16 + 2, TEXT_SKIP_DRAW, NULL);
    }
...
{
    AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL, sText_CallingDots, 32, 2, 1, NULL);
}
...
{
    AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL, gText_TrainerCloseBy, 32, 2, 1, NULL);
}
...
static void PrintMatchCallMessage(struct Pokenav_MatchCallGfx *gfx)
{
    ...
    AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL, str, 32, 2, speed, NULL);
}
