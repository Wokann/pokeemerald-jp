...
// JP note: the EWRAM pointer is bound via ld alias (0x02039DDC) instead of a
// static EWRAM_DATA definition, which would land in a discarded section.
extern struct AnimStatsChangeData *sAnimStatsChangeData;
...
void AnimTask_BlendBattleAnimPalExclude(u8 taskId)
{
    ...
    animBattlers[1] = 0xFF;
    selectedPalettes = UnpackSelectedBattleAnimPalettes(F_PAL_BG);
