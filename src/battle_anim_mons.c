...
extern union AffineAnimCmd *sAnimTaskAffineAnim;
...
// JP byte-exact: agbcc emits `adds r1, r1, r0` for `y2 += Sin(...)` but
// the JP ROM uses `adds r0, r0, r1`; keep naked asm for the 0x34-byte
// bodies (0x080A61C8/0x080A61FC).
__attribute__((naked)) bool8 TranslateAnimHorizontalArc(struct Sprite *sprite)
{
...
__attribute__((naked)) bool8 TranslateAnimVerticalArc(struct Sprite *sprite)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "push {r4, lr}\n\t"
...
// JP byte-exact: agbcc folds away the redundant `mov r0, r8` the ROM has
// when normalizing the CreateInvisibleSpriteWithCallback result to u8;
// keep naked asm to reproduce the 0xA4-byte function and its literal
// pool position (0x080A8290-0x080A8334).
__attribute__((naked)) u8 CreateInvisibleSpriteCopy(int battler, u8 spriteId, int species)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "push {r4, r5, r6, lr}\n\t"
        ...
        "bx r1\n\t"
        ".align 2, 0\n\t"
        "_080A8328: .4byte SpriteCallbackDummy\n\t"
        "_080A832C: .4byte gSprites\n\t"
...
void AnimTask_AlphaFadeIn(u8 taskId)
{
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA, (u16)BLDALPHA_BLEND(gBattleAnimArgs[0], gBattleAnimArgs[1]));
...
static void AnimTask_AlphaFadeIn_Step(u8 taskId)
{
    ...
    if (++task->data[0] > task->data[1])
    {
        ...
        SetGpuReg(REG_OFFSET_BLDALPHA, (u16)BLDALPHA_BLEND(task->data[3], task->data[4]));
...
static void AnimTask_BlendMonInAndOut_Step(u8 taskId)
{
    ...
    if (++task->data[4] >= task->data[5])
    {
        ...
        {
            task->data[2]++;
            BlendPalette((u16)task->data[0], 15, (u8)task->data[2], (u16)task->data[1]);
...
// Create Pok̩mon sprite to be used for a move animation effect (e.g. Role Play / Snatch)
u8 CreateAdditionalMonSpriteForMoveAnim(u16 species, bool8 isBackpic, u8 id, s16 x, s16 y, u8 subpriority, u32 personality, u32 trainerId, u32 battler, bool32 ignoreDeoxysForm)
{
    u8 spriteId;
