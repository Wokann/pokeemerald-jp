...
// JP-specific direction->walk action lookup with bounds check
// (pokeemerald inlines this into GetWalkNormalMovementAction)
extern u8 sub_08092CF8(u32 direction);
...
static void Task_ExitNonAnimDoor(u8 taskId)
{
    ...
    case 1:
        if (WaitForWeatherFadeIn())
        {
            ...
            SetPlayerVisibility(TRUE);
            objEventId = GetObjectEventIdByLocalIdAndMap(LOCALID_PLAYER, 0, 0);
            ObjectEventSetHeldMovement(&gObjectEvents[objEventId], sub_08092CF8(GetPlayerFacingDirection()));
            task->tState = 2;
        }
        break;
    case 2:
