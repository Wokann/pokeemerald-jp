...
// JP ROM-resident RAM (bound in sym_iwram_jp.txt / sym_ewram_jp.txt).
extern EWRAM_DATA struct YesNoFuncTable sYesNo;
...
{
    CreateYesNoMenuAtPos(template, unused1, unused2, unused3, tileStart, palette, 0);
    sYesNo = *yesNo;
    gTasks[taskId].func = Task_CallYesOrNoCallback;
}
