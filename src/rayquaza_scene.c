#include "global.h"
#include "rayquaza_scene.h"
#include "sprite.h"
#include "bg.h"
#include "graphics.h"
#include "scanline_effect.h"


#define TAG_DUOFIGHT_GROUDON             30505
#define TAG_DUOFIGHT_GROUDON_SHOULDER    30506
#define TAG_DUOFIGHT_GROUDON_CLAW        30507
#define TAG_DUOFIGHT_KYOGRE              30508
#define TAG_DUOFIGHT_KYOGRE_PECTORAL_FIN 30509
#define TAG_DUOFIGHT_KYOGRE_DORSAL_FIN   30510
#define TAG_FLIGHT_SMOKE                 30555
#define TAG_DESCENDS_RAYQUAZA            30556
#define TAG_DESCENDS_RAYQUAZA_TAIL       30557
#define TAG_CHASE_GROUDON                 30565
#define TAG_CHASE_GROUDON_TAIL            30566
#define TAG_CHASE_KYOGRE                  30568
#define TAG_CHASE_RAYQUAZA                30569
#define TAG_CHASE_RAYQUAZA_TAIL           30570
#define TAG_CHASE_SPLASH                  30571

#define MAX_SMOKE 10


void Task_DuoFightAnim(void);
void Task_RayTakesFlightAnim(void);
void Task_RayDescendsAnim(void);
void Task_RayChargesAnim(void);
void Task_RayChasesAwayAnim(void);
void Task_EndAfterFadeScreen(void);
void SpriteCB_TakesFlight_Smoke(struct Sprite *sprite);
void SpriteCB_ChasesAway_Rayquaza(struct Sprite *sprite);

static void (*const sTasksForAnimations[])(void) __attribute__((section(".rodata.rayquaza_scene_tasks"))) =
{
    Task_DuoFightAnim,
    Task_DuoFightAnim,
    Task_RayTakesFlightAnim,
    Task_RayDescendsAnim,
    Task_RayChargesAnim,
    Task_RayChasesAwayAnim,
    Task_EndAfterFadeScreen,
};

#define RAYQUAZA_SCENE_OAM(name, dimensions) \
const struct OamData name __attribute__((section(".rodata.rayquaza_scene_oam"))) = \
{ \
    .shape = SPRITE_SHAPE(dimensions), \
    .size = SPRITE_SIZE(dimensions), \
    .priority = 2, \
}

RAYQUAZA_SCENE_OAM(sOam_64x64, 64x64);
RAYQUAZA_SCENE_OAM(sOam_32x32, 32x32);
RAYQUAZA_SCENE_OAM(sOam_64x32, 64x32);
RAYQUAZA_SCENE_OAM(sOam_32x16, 32x16);
RAYQUAZA_SCENE_OAM(sOam_16x8, 16x8);
RAYQUAZA_SCENE_OAM(sOam_16x32, 16x32);
RAYQUAZA_SCENE_OAM(sOam_16x16, 16x16);
RAYQUAZA_SCENE_OAM(sOam_32x8, 32x8);

#undef RAYQUAZA_SCENE_OAM

#define RAYQUAZA_SCENE_DUO_PRE_DATA __attribute__((section(".rodata.rayquaza_scene_duo_pre")))

const union AnimCmd sAnim_DuoFightPre_Groudon_Head[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(0, 30),
    ANIMCMD_FRAME(64, 30),
    ANIMCMD_FRAME(128, 30),
    ANIMCMD_FRAME(64, 30),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFightPre_Groudon_Body[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(192, 30),
    ANIMCMD_FRAME(256, 30),
    ANIMCMD_FRAME(320, 30),
    ANIMCMD_FRAME(256, 30),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_DuoFightPre_Groudon[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    sAnim_DuoFightPre_Groudon_Head,
    sAnim_DuoFightPre_Groudon_Body
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_Groudon RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_64x64,
    .anims = sAnims_DuoFightPre_Groudon,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFightPre_GroudonShoulderKyogreDorsalFin[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd *const sAnims_DuoFightPre_GroudonShoulderKyogreDorsalFin[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    sAnim_DuoFightPre_GroudonShoulderKyogreDorsalFin
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_GroudonShoulder RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON_SHOULDER,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_32x32,
    .anims = sAnims_DuoFightPre_GroudonShoulderKyogreDorsalFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFightPre_GroudonClaw[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd *const sAnims_DuoFightPre_GroudonClaw[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    sAnim_DuoFightPre_GroudonClaw
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_GroudonClaw RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON_CLAW,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_64x32,
    .anims = sAnims_DuoFightPre_GroudonClaw,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_TopLeft[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_TopRight[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(8, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_FaceLeft[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(16, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_FaceRight[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(24, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_ChinLeft[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(32, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_ChinRight[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(40, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_LeftPectoralFin[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(48, 36),
    ANIMCMD_FRAME(64, 36),
    ANIMCMD_FRAME(80, 36),
    ANIMCMD_FRAME(64, 36),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_LeftShoulder[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(56, 36),
    ANIMCMD_FRAME(72, 36),
    ANIMCMD_FRAME(88, 36),
    ANIMCMD_FRAME(72, 36),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFightPre_Kyogre_RightShoulder[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(96, 36),
    ANIMCMD_FRAME(104, 36),
    ANIMCMD_FRAME(112, 36),
    ANIMCMD_FRAME(104, 36),
    ANIMCMD_JUMP(0),
};

// Edge of the right pectoral fin is its own sprite (see KyogrePectoralFin)
const union AnimCmd *const sAnims_DuoFightPre_Kyogre[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    sAnim_DuoFightPre_Kyogre_TopLeft,
    sAnim_DuoFightPre_Kyogre_TopRight,
    sAnim_DuoFightPre_Kyogre_FaceLeft,
    sAnim_DuoFightPre_Kyogre_FaceRight,
    sAnim_DuoFightPre_Kyogre_ChinLeft,
    sAnim_DuoFightPre_Kyogre_ChinRight,
    sAnim_DuoFightPre_Kyogre_LeftPectoralFin,
    sAnim_DuoFightPre_Kyogre_LeftShoulder,
    sAnim_DuoFightPre_Kyogre_RightShoulder
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_Kyogre RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_32x16,
    .anims = sAnims_DuoFightPre_Kyogre,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFightPre_KyogrePectoralFin[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    ANIMCMD_FRAME(0, 36),
    ANIMCMD_FRAME(2, 36),
    ANIMCMD_FRAME(4, 36),
    ANIMCMD_FRAME(2, 36),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_DuoFightPre_KyogrePectoralFin[] RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    sAnim_DuoFightPre_KyogrePectoralFin
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_KyogrePectoralFin RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE_PECTORAL_FIN,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_16x8,
    .anims = sAnims_DuoFightPre_KyogrePectoralFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const struct SpriteTemplate sSpriteTemplate_DuoFightPre_KyogreDorsalFin RAYQUAZA_SCENE_DUO_PRE_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE_DORSAL_FIN,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_32x32,
    .anims = sAnims_DuoFightPre_GroudonShoulderKyogreDorsalFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

#undef RAYQUAZA_SCENE_DUO_PRE_DATA

#define RAYQUAZA_SCENE_DUO_DATA __attribute__((section(".rodata.rayquaza_scene_duo")))

const struct ScanlineEffectParams sScanlineParams_DuoFight_Clouds RAYQUAZA_SCENE_DUO_DATA =
{
    .dmaDest = &REG_BG1HOFS,
    .dmaControl = SCANLINE_EFFECT_DMACNT_16BIT,
    .initState = 1
};

const struct BgTemplate sBgTemplates_DuoFight[] RAYQUAZA_SCENE_DUO_DATA =
{
    {
        .bg = 0,
        .charBaseIndex = 0,
        .mapBaseIndex = 31,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 0,
        .baseTile = 0
    },
    {
        .bg = 1,
        .charBaseIndex = 0,
        .mapBaseIndex = 30,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 2,
        .baseTile = 0
    },
    {
        .bg = 2,
        .charBaseIndex = 0,
        .mapBaseIndex = 29,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 1,
        .baseTile = 0
    },
};

const union AnimCmd sAnim_DuoFight_Groudon_Head[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(0, 20),
    ANIMCMD_FRAME(64, 20),
    ANIMCMD_FRAME(128, 20),
    ANIMCMD_FRAME(64, 20),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFight_Groudon_Body[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(192, 20),
    ANIMCMD_FRAME(256, 20),
    ANIMCMD_FRAME(320, 20),
    ANIMCMD_FRAME(256, 20),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_DuoFight_Groudon[] RAYQUAZA_SCENE_DUO_DATA =
{
    sAnim_DuoFight_Groudon_Head,
    sAnim_DuoFight_Groudon_Body
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_Groudon RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_Groudon_Gfx, 0x3000, TAG_DUOFIGHT_GROUDON
};

const struct CompressedSpritePalette sSpritePal_DuoFight_Groudon RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_Groudon_Pal, TAG_DUOFIGHT_GROUDON
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_Groudon RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_64x64,
    .anims = sAnims_DuoFight_Groudon,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFight_GroudonShoulderKyogreDorsalFin[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd *const sAnims_DuoFight_GroudonShoulderKyogreDorsalFin[] RAYQUAZA_SCENE_DUO_DATA =
{
    sAnim_DuoFight_GroudonShoulderKyogreDorsalFin
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_GroudonShoulder RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_GroudonShoulder_Gfx, 0x200, TAG_DUOFIGHT_GROUDON_SHOULDER
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_GroudonShoulder RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON_SHOULDER,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_32x32,
    .anims = sAnims_DuoFight_GroudonShoulderKyogreDorsalFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFight_GroudonClaw[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd *const sAnims_DuoFight_GroudonClaw[] RAYQUAZA_SCENE_DUO_DATA =
{
    sAnim_DuoFight_GroudonClaw
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_GroudonClaw RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_GroudonClaw_Gfx, 0x400, TAG_DUOFIGHT_GROUDON_CLAW
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_GroudonClaw RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_GROUDON_CLAW,
    .paletteTag = TAG_DUOFIGHT_GROUDON,
    .oam = &sOam_64x32,
    .anims = sAnims_DuoFight_GroudonClaw,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFight_Kyogre_TopLeft[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_TopRight[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(8, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_FaceLeft[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(16, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_FaceRight[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(24, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_ChinLeft[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(32, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_ChinRight[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(40, 1),
    ANIMCMD_END
};

const union AnimCmd sAnim_DuoFight_Kyogre_LeftPectoralFin[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(48, 24),
    ANIMCMD_FRAME(64, 24),
    ANIMCMD_FRAME(80, 24),
    ANIMCMD_FRAME(64, 24),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFight_Kyogre_LeftShoulder[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(56, 24),
    ANIMCMD_FRAME(72, 24),
    ANIMCMD_FRAME(88, 24),
    ANIMCMD_FRAME(72, 24),
    ANIMCMD_JUMP(0),
};

const union AnimCmd sAnim_DuoFight_Kyogre_RightShoulder[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(96, 24),
    ANIMCMD_FRAME(104, 24),
    ANIMCMD_FRAME(112, 24),
    ANIMCMD_FRAME(104, 24),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_DuoFight_Kyogre[] RAYQUAZA_SCENE_DUO_DATA =
{
    sAnim_DuoFight_Kyogre_TopLeft,
    sAnim_DuoFight_Kyogre_TopRight,
    sAnim_DuoFight_Kyogre_FaceLeft,
    sAnim_DuoFight_Kyogre_FaceRight,
    sAnim_DuoFight_Kyogre_ChinLeft,
    sAnim_DuoFight_Kyogre_ChinRight,
    sAnim_DuoFight_Kyogre_LeftPectoralFin,
    sAnim_DuoFight_Kyogre_LeftShoulder,
    sAnim_DuoFight_Kyogre_RightShoulder
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_Kyogre RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_Kyogre_Gfx, 0xF00, TAG_DUOFIGHT_KYOGRE
};

const struct CompressedSpritePalette sSpritePal_DuoFight_Kyogre RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_Kyogre_Pal, TAG_DUOFIGHT_KYOGRE
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_Kyogre RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_32x16,
    .anims = sAnims_DuoFight_Kyogre,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const union AnimCmd sAnim_DuoFight_KyogrePectoralFin[] RAYQUAZA_SCENE_DUO_DATA =
{
    ANIMCMD_FRAME(0, 24),
    ANIMCMD_FRAME(2, 24),
    ANIMCMD_FRAME(4, 24),
    ANIMCMD_FRAME(2, 24),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_DuoFight_KyogrePectoralFin[] RAYQUAZA_SCENE_DUO_DATA =
{
    sAnim_DuoFight_KyogrePectoralFin
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_KyogrePectoralFin RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_KyogrePectoralFin_Gfx, 0xC0, TAG_DUOFIGHT_KYOGRE_PECTORAL_FIN
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_KyogrePectoralFin RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE_PECTORAL_FIN,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_16x8,
    .anims = sAnims_DuoFight_KyogrePectoralFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const struct CompressedSpriteSheet sSpriteSheet_DuoFight_KyogreDorsalFin RAYQUAZA_SCENE_DUO_DATA =
{
    gRaySceneDuoFight_KyogreDorsalFin_Gfx, 0x200, TAG_DUOFIGHT_KYOGRE_DORSAL_FIN
};

const struct SpriteTemplate sSpriteTemplate_DuoFight_KyogreDorsalFin RAYQUAZA_SCENE_DUO_DATA =
{
    .tileTag = TAG_DUOFIGHT_KYOGRE_DORSAL_FIN,
    .paletteTag = TAG_DUOFIGHT_KYOGRE,
    .oam = &sOam_32x32,
    .anims = sAnims_DuoFight_GroudonShoulderKyogreDorsalFin,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

#undef RAYQUAZA_SCENE_DUO_DATA

#define RAYQUAZA_SCENE_TAKES_FLIGHT_DATA __attribute__((section(".rodata.rayquaza_scene_takes_flight")))

const struct BgTemplate sBgTemplates_TakesFlight[] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    {
        .bg = 0,
        .charBaseIndex = 2,
        .mapBaseIndex = 31,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 0,
        .baseTile = 0
    },
    {
        .bg = 1,
        .charBaseIndex = 0,
        .mapBaseIndex = 30,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 2,
        .baseTile = 0
    },
    {
        .bg = 2,
        .charBaseIndex = 1,
        .mapBaseIndex = 29,
        .screenSize = 1,
        .paletteMode = 0,
        .priority = 1,
        .baseTile = 0
    }
};

const union AnimCmd sAnim_TakesFlight_Smoke[] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    ANIMCMD_FRAME(0, 1),
    ANIMCMD_END
};

const union AnimCmd *const sAnims_TakesFlight_Smoke[] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    sAnim_TakesFlight_Smoke
};

const union AffineAnimCmd sAffineAnim_TakesFlight_Smoke[] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    AFFINEANIMCMD_FRAME(-64, -64, 0, 1),
    AFFINEANIMCMD_FRAME(32, 32, 0, 14),
    AFFINEANIMCMD_FRAME(256, 256, 0, 0),
    AFFINEANIMCMD_JUMP(0)
};

const union AffineAnimCmd *const sAffineAnims_TakesFlight_Smoke[] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    sAffineAnim_TakesFlight_Smoke
};

const struct CompressedSpriteSheet sSpriteSheet_TakesFlight_Smoke RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    gRaySceneTakesFlight_Smoke_Gfx, 0x100, TAG_FLIGHT_SMOKE
};

const struct CompressedSpritePalette sSpritePal_TakesFlight_Smoke RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    gRaySceneTakesFlight_Smoke_Pal, TAG_FLIGHT_SMOKE
};

const struct SpriteTemplate sSpriteTemplate_TakesFlight_Smoke RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    .tileTag = TAG_FLIGHT_SMOKE,
    .paletteTag = TAG_FLIGHT_SMOKE,
    .oam = &sOam_32x16,
    .anims = sAnims_TakesFlight_Smoke,
    .images = NULL,
    .affineAnims = sAffineAnims_TakesFlight_Smoke,
    .callback = SpriteCB_TakesFlight_Smoke,
};

const s8 sTakesFlight_SmokeCoords[MAX_SMOKE][2] RAYQUAZA_SCENE_TAKES_FLIGHT_DATA =
{
    {-1,  5},
    {-3, -4},
    { 5, -3},
    {-7,  2},
    {-9, -1},
    { 1, -5},
    { 3,  4},
    {-5,  3},
    { 7, -2},
    { 9,  1}
};

#undef RAYQUAZA_SCENE_TAKES_FLIGHT_DATA

#define RAYQUAZA_SCENE_DESCENDS_DATA __attribute__((section(".rodata.rayquaza_scene_descends")))

const struct BgTemplate sBgTemplates_Descends[] RAYQUAZA_SCENE_DESCENDS_DATA =
{
    {
        .bg = 0,
        .charBaseIndex = 0,
        .mapBaseIndex = 31,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 0,
        .baseTile = 0
    },
    {
        .bg = 1,
        .charBaseIndex = 1,
        .mapBaseIndex = 30,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 1,
        .baseTile = 0
    },
    {
        .bg = 2,
        .charBaseIndex = 2,
        .mapBaseIndex = 29,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 2,
        .baseTile = 0
    },
    {
        .bg = 3,
        .charBaseIndex = 1,
        .mapBaseIndex = 28,
        .screenSize = 0,
        .paletteMode = 0,
        .priority = 3,
        .baseTile = 0
    }
};

const union AnimCmd sAnim_Descends_Rayquaza[] RAYQUAZA_SCENE_DESCENDS_DATA =
{
    ANIMCMD_FRAME(0, 32),
    ANIMCMD_FRAME(64, 32),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_Descends_Rayquaza[] RAYQUAZA_SCENE_DESCENDS_DATA =
{
    sAnim_Descends_Rayquaza
};

const union AnimCmd sAnim_Descends_RayquazaTail[] RAYQUAZA_SCENE_DESCENDS_DATA =
{
    ANIMCMD_FRAME(0, 32),
    ANIMCMD_FRAME(8, 32),
    ANIMCMD_JUMP(0),
};

const union AnimCmd *const sAnims_Descends_RayquazaTail[] RAYQUAZA_SCENE_DESCENDS_DATA =
{
    sAnim_Descends_RayquazaTail
};

const struct CompressedSpriteSheet sSpriteSheet_Descends_Rayquaza RAYQUAZA_SCENE_DESCENDS_DATA =
{
    gRaySceneDescends_Rayquaza_Gfx, 0x1000, TAG_DESCENDS_RAYQUAZA
};

const struct CompressedSpriteSheet sSpriteSheet_Descends_RayquazaTail RAYQUAZA_SCENE_DESCENDS_DATA =
{
    gRaySceneDescends_RayquazaTail_Gfx, 0x200, TAG_DESCENDS_RAYQUAZA_TAIL
};

const struct CompressedSpritePalette sSpritePal_Descends_Rayquaza RAYQUAZA_SCENE_DESCENDS_DATA =
{
    gRaySceneTakesFlight_Rayquaza_Pal, TAG_DESCENDS_RAYQUAZA // "Takes flight" palette re-used here
};

const struct SpriteTemplate sSpriteTemplate_Descends_Rayquaza RAYQUAZA_SCENE_DESCENDS_DATA =
{
    .tileTag = TAG_DESCENDS_RAYQUAZA,
    .paletteTag = TAG_DESCENDS_RAYQUAZA,
    .oam = &sOam_64x64,
    .anims = sAnims_Descends_Rayquaza,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

const struct SpriteTemplate sSpriteTemplate_Descends_RayquazaTail RAYQUAZA_SCENE_DESCENDS_DATA =
{
    .tileTag = TAG_DESCENDS_RAYQUAZA_TAIL,
    .paletteTag = TAG_DESCENDS_RAYQUAZA,
    .oam = &sOam_16x32,
    .anims = sAnims_Descends_RayquazaTail,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy,
};

#undef RAYQUAZA_SCENE_DESCENDS_DATA

#define RAYQUAZA_SCENE_CHARGES_DATA __attribute__((section(".rodata.rayquaza_scene_charges")))

const struct BgTemplate sBgTemplates_Charges[] RAYQUAZA_SCENE_CHARGES_DATA =
{
    { .bg = 0, .charBaseIndex = 2, .mapBaseIndex = 31, .screenSize = 0, .paletteMode = 0, .priority = 0, .baseTile = 0 },
    { .bg = 1, .charBaseIndex = 1, .mapBaseIndex = 30, .screenSize = 0, .paletteMode = 0, .priority = 1, .baseTile = 0 },
    { .bg = 2, .charBaseIndex = 2, .mapBaseIndex = 29, .screenSize = 0, .paletteMode = 0, .priority = 2, .baseTile = 0 },
    { .bg = 3, .charBaseIndex = 3, .mapBaseIndex = 28, .screenSize = 0, .paletteMode = 0, .priority = 3, .baseTile = 0 }
};

#undef RAYQUAZA_SCENE_CHARGES_DATA

#define RAYQUAZA_SCENE_CHASES_AWAY_DATA __attribute__((section(".rodata.rayquaza_scene_chases_away")))

const union AnimCmd sAnim_ChasesAway_Groudon_Still[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    ANIMCMD_FRAME(0, 1), ANIMCMD_END
};

const union AnimCmd sAnim_ChasesAway_Groudon_Moving[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    ANIMCMD_FRAME(0, 48), ANIMCMD_FRAME(64, 32), ANIMCMD_FRAME(0, 48),
    ANIMCMD_FRAME(128, 32), ANIMCMD_JUMP(0)
};

const union AnimCmd *const sAnims_ChasesAway_Groudon[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_Groudon_Still, sAnim_ChasesAway_Groudon_Moving
};

const union AnimCmd sAnim_ChasesAway_GroudonTail[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    ANIMCMD_FRAME(0, 1), ANIMCMD_END
};

const union AnimCmd *const sAnims_ChasesAway_GroudonTail[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_GroudonTail
};

const union AnimCmd sAnim_ChasesAway_Kyogre_Front[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_Kyogre_Back[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(16, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_Kyogre_Tail[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(32, 1), ANIMCMD_END };

const union AnimCmd *const sAnims_ChasesAway_Kyogre[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_Kyogre_Front, sAnim_ChasesAway_Kyogre_Back, sAnim_ChasesAway_Kyogre_Tail
};

const union AnimCmd sAnim_ChasesAway_Rayquaza_FlyingDown[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_Rayquaza_Arriving[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(64, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_Rayquaza_Floating[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(128, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_Rayquaza_Shouting[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(192, 1), ANIMCMD_END };

const union AnimCmd *const sAnims_ChasesAway_Rayquaza[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_Rayquaza_FlyingDown, sAnim_ChasesAway_Rayquaza_Arriving,
    sAnim_ChasesAway_Rayquaza_Floating, sAnim_ChasesAway_Rayquaza_Shouting
};

const union AnimCmd sAnim_ChasesAway_RayquazaTail_FlyingDown[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(0, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_RayquazaTail_Arriving[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(16, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_RayquazaTail_Floating[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(32, 1), ANIMCMD_END };
const union AnimCmd sAnim_ChasesAway_RayquazaTail_Shouting[] RAYQUAZA_SCENE_CHASES_AWAY_DATA = { ANIMCMD_FRAME(48, 1), ANIMCMD_END };

const union AnimCmd *const sAnims_ChasesAway_RayquazaTail[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_RayquazaTail_FlyingDown, sAnim_ChasesAway_RayquazaTail_Arriving,
    sAnim_ChasesAway_RayquazaTail_Floating, sAnim_ChasesAway_RayquazaTail_Shouting
};

const union AnimCmd sAnim_ChasesAway_KyogreSplash[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    ANIMCMD_FRAME(0, 8), ANIMCMD_FRAME(4, 8), ANIMCMD_FRAME(8, 8),
    ANIMCMD_FRAME(12, 8), ANIMCMD_FRAME(16, 8), ANIMCMD_FRAME(20, 8), ANIMCMD_JUMP(0)
};

const union AnimCmd *const sAnims_ChasesAway_KyogreSplash[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    sAnim_ChasesAway_KyogreSplash
};

#define CHASE_SHEET(name, gfx, size, tag) \
const struct CompressedSpriteSheet name RAYQUAZA_SCENE_CHASES_AWAY_DATA = { gfx, size, tag }

CHASE_SHEET(sSpriteSheet_ChasesAway_Groudon, gRaySceneChasesAway_Groudon_Gfx, 0x1800, TAG_CHASE_GROUDON);
CHASE_SHEET(sSpriteSheet_ChasesAway_GroudonTail, gRaySceneChasesAway_GroudonTail_Gfx, 0x80, TAG_CHASE_GROUDON_TAIL);
CHASE_SHEET(sSpriteSheet_ChasesAway_Kyogre, gRaySceneChasesAway_Kyogre_Gfx, 0x600, TAG_CHASE_KYOGRE);
CHASE_SHEET(sSpriteSheet_ChasesAway_Rayquaza, gRaySceneChasesAway_Rayquaza_Gfx, 0x2000, TAG_CHASE_RAYQUAZA);
CHASE_SHEET(sSpriteSheet_ChasesAway_RayquazaTail, gRaySceneChasesAway_RayquazaTail_Gfx, 0x800, TAG_CHASE_RAYQUAZA_TAIL);
CHASE_SHEET(sSpriteSheet_ChasesAway_KyogreSplash, gRaySceneChasesAway_KyogreSplash_Gfx, 0x300, TAG_CHASE_SPLASH);

#undef CHASE_SHEET

#define CHASE_PAL(name, pal, tag) \
const struct CompressedSpritePalette name RAYQUAZA_SCENE_CHASES_AWAY_DATA = { pal, tag }

CHASE_PAL(sSpritePal_ChasesAway_Groudon, gRaySceneChasesAway_Groudon_Pal, TAG_CHASE_GROUDON);
CHASE_PAL(sSpritePal_ChasesAway_Kyogre, gRaySceneChasesAway_Kyogre_Pal, TAG_CHASE_KYOGRE);
CHASE_PAL(sSpritePal_ChasesAway_Rayquaza, gRaySceneChasesAway_Rayquaza_Pal, TAG_CHASE_RAYQUAZA);
CHASE_PAL(sSpritePal_ChasesAway_KyogreSplash, gRaySceneChasesAway_KyogreSplash_Pal, TAG_CHASE_SPLASH);

#undef CHASE_PAL

#define CHASE_TEMPLATE(name, tileTag_, palTag_, oam_, anims_, callback_) \
const struct SpriteTemplate name RAYQUAZA_SCENE_CHASES_AWAY_DATA = \
{ \
    .tileTag = tileTag_, .paletteTag = palTag_, .oam = &oam_, .anims = anims_, .images = NULL, \
    .affineAnims = gDummySpriteAffineAnimTable, .callback = callback_ \
}

CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_Groudon, TAG_CHASE_GROUDON, TAG_CHASE_GROUDON, sOam_64x64, sAnims_ChasesAway_Groudon, SpriteCallbackDummy);
CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_GroudonTail, TAG_CHASE_GROUDON_TAIL, TAG_CHASE_GROUDON, sOam_16x16, sAnims_ChasesAway_GroudonTail, SpriteCallbackDummy);
CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_Kyogre, TAG_CHASE_KYOGRE, TAG_CHASE_KYOGRE, sOam_32x32, sAnims_ChasesAway_Kyogre, SpriteCallbackDummy);
CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_Rayquaza, TAG_CHASE_RAYQUAZA, TAG_CHASE_RAYQUAZA, sOam_64x64, sAnims_ChasesAway_Rayquaza, SpriteCB_ChasesAway_Rayquaza);
CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_RayquazaTail, TAG_CHASE_RAYQUAZA_TAIL, TAG_CHASE_RAYQUAZA, sOam_32x32, sAnims_ChasesAway_RayquazaTail, SpriteCallbackDummy);
CHASE_TEMPLATE(sSpriteTemplate_ChasesAway_KyogreSplash, TAG_CHASE_SPLASH, TAG_CHASE_SPLASH, sOam_32x8, sAnims_ChasesAway_KyogreSplash, SpriteCallbackDummy);

#undef CHASE_TEMPLATE

const struct BgTemplate sBgTemplates_ChasesAway[] RAYQUAZA_SCENE_CHASES_AWAY_DATA =
{
    { .bg = 0, .charBaseIndex = 1, .mapBaseIndex = 31, .screenSize = 0, .paletteMode = 0, .priority = 1, .baseTile = 0 },
    { .bg = 1, .charBaseIndex = 1, .mapBaseIndex = 30, .screenSize = 0, .paletteMode = 0, .priority = 2, .baseTile = 0 },
    { .bg = 2, .charBaseIndex = 0, .mapBaseIndex = 29, .screenSize = 0, .paletteMode = 0, .priority = 0, .baseTile = 0 }
};

#undef RAYQUAZA_SCENE_CHASES_AWAY_DATA

__attribute__((naked)) void DoRayquazaScene(u8 animId, bool8 endEarly, MainCallback exitCallback)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	adds r4, r0, #0\n\t"
        "	adds r5, r1, #0\n\t"
        "	mov r8, r2\n\t"
        "	lsls r4, r4, #0x18\n\t"
        "	lsrs r4, r4, #0x18\n\t"
        "	lsls r5, r5, #0x18\n\t"
        "	lsrs r5, r5, #0x18\n\t"
        "	ldr r6, _081D609C\n\t"
        "	ldr r0, _081D60A0\n\t"
        "	bl AllocZeroed\n\t"
        "	str r0, [r6]\n\t"
        "	ldr r1, _081D60A4\n\t"
        "	adds r0, r0, r1\n\t"
        "	strb r4, [r0]\n\t"
        "	ldr r0, [r6]\n\t"
        "	mov r1, r8\n\t"
        "	str r1, [r0]\n\t"
        "	ldr r1, _081D60A8\n\t"
        "	adds r0, r0, r1\n\t"
        "	strb r5, [r0]\n\t"
        "	ldr r0, _081D60AC\n\t"
        "	bl SetMainCallback2\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D609C: .4byte gUnknown_203CC2C\n\t"
        "_081D60A0: .4byte 0x00002018\n\t"
        "_081D60A4: .4byte 0x00002006\n\t"
        "_081D60A8: .4byte 0x00002007\n\t"
        "_081D60AC: .4byte CB2_InitRayquazaScene + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void CB2_InitRayquazaScene(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	bl SetVBlankHBlankCallbacksToNull\n\t"
        "	bl ClearScheduledBgCopiesToVram\n\t"
        "	bl ScanlineEffect_Stop\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	bl ResetPaletteFade\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl ResetTasks\n\t"
        "	movs r0, #0\n\t"
        "	movs r1, #0xf0\n\t"
        "	movs r2, #0x20\n\t"
        "	bl FillPalette\n\t"
        "	ldr r1, _081D60FC\n\t"
        "	ldr r0, _081D6100\n\t"
        "	ldr r0, [r0]\n\t"
        "	ldr r2, _081D6104\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrb r0, [r0]\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r0, [r0]\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	ldr r0, _081D6108\n\t"
        "	bl SetMainCallback2\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D60FC: .4byte sTasksForAnimations\n\t"
        "_081D6100: .4byte gUnknown_203CC2C\n\t"
        "_081D6104: .4byte 0x00002006\n\t"
        "_081D6108: .4byte CB2_RayquazaScene + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void CB2_RayquazaScene(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	bl RunTasks\n\t"
        "	bl AnimateSprites\n\t"
        "	bl BuildOamBuffer\n\t"
        "	bl DoScheduledBgTilemapCopiesToVram\n\t"
        "	bl UpdatePaletteFade\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void VBlankCB_RayquazaScene(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	bl LoadOam\n\t"
        "	bl ProcessSpriteCopyRequests\n\t"
        "	bl TransferPlttBuffer\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_EndAfterFadeScreen(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	ldr r0, _081D6174\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D616C\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	ldr r4, _081D6178\n\t"
        "	ldr r0, [r4]\n\t"
        "	ldr r0, [r0]\n\t"
        "	bl SetMainCallback2\n\t"
        "	ldr r0, [r4]\n\t"
        "	bl Free\n\t"
        "	adds r0, r5, #0\n\t"
        "	bl DestroyTask\n\t"
        "_081D616C:\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6174: .4byte gPaletteFade\n\t"
        "_081D6178: .4byte gUnknown_203CC2C\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_SetNextAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r3, r0, #0x18\n\t"
        "	adds r6, r3, #0\n\t"
        "	ldr r0, _081D61B0\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	cmp r5, #0\n\t"
        "	bne _081D61EE\n\t"
        "	ldr r2, _081D61B4\n\t"
        "	ldr r1, [r2]\n\t"
        "	ldr r4, _081D61B8\n\t"
        "	adds r0, r1, r4\n\t"
        "	ldrb r0, [r0]\n\t"
        "	cmp r0, #1\n\t"
        "	bne _081D61C4\n\t"
        "	ldr r0, _081D61BC\n\t"
        "	lsls r1, r3, #2\n\t"
        "	adds r1, r1, r3\n\t"
        "	lsls r1, r1, #3\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r0, _081D61C0\n\t"
        "	b _081D61EC\n\t"
        "	.align 2, 0\n\t"
        "_081D61B0: .4byte gPaletteFade\n\t"
        "_081D61B4: .4byte gUnknown_203CC2C\n\t"
        "_081D61B8: .4byte 0x00002007\n\t"
        "_081D61BC: .4byte gTasks\n\t"
        "_081D61C0: .4byte Task_EndAfterFadeScreen + 1\n\t"
        "_081D61C4:\n\t"
        "	ldr r4, _081D61F4\n\t"
        "	adds r1, r1, r4\n\t"
        "	ldrb r0, [r1]\n\t"
        "	adds r0, #1\n\t"
        "	strb r0, [r1]\n\t"
        "	ldr r2, [r2]\n\t"
        "	ldr r1, _081D61F8\n\t"
        "	adds r0, r2, r1\n\t"
        "	strh r5, [r0]\n\t"
        "	ldr r0, _081D61FC\n\t"
        "	lsls r1, r6, #2\n\t"
        "	adds r1, r1, r6\n\t"
        "	lsls r1, r1, #3\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r3, _081D6200\n\t"
        "	adds r2, r2, r4\n\t"
        "	ldrb r0, [r2]\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldr r0, [r0]\n\t"
        "_081D61EC:\n\t"
        "	str r0, [r1]\n\t"
        "_081D61EE:\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D61F4: .4byte 0x00002006\n\t"
        "_081D61F8: .4byte 0x00002004\n\t"
        "_081D61FC: .4byte gTasks\n\t"
        "_081D6200: .4byte sTasksForAnimations\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6204(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	movs r0, #0x48\n\t"
        "	movs r1, #0x3f\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0x4a\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0x40\n\t"
        "	movs r1, #0xf0\n\t"
        "	bl SetGpuReg\n\t"
        "	ldr r1, _081D6234\n\t"
        "	movs r0, #0x44\n\t"
        "	bl SetGpuReg\n\t"
        "	ldr r0, _081D6238\n\t"
        "	movs r1, #0\n\t"
        "	strh r1, [r0]\n\t"
        "	ldr r0, _081D623C\n\t"
        "	strh r1, [r0]\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6234: .4byte 0x00001888\n\t"
        "_081D6238: .4byte gPlttBufferUnfaded\n\t"
        "_081D623C: .4byte gPlttBufferFaded\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6240(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	movs r0, #0x48\n\t"
        "	movs r1, #0x3f\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0x4a\n\t"
        "	movs r1, #0x3f\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleDuoFightPre(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	adds r4, r0, #0\n\t"
        "	lsls r0, r4, #2\n\t"
        "	adds r0, r0, r4\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D6288\n\t"
        "	adds r5, r0, r1\n\t"
        "	bl sub_081D6E48\n\t"
        "	ldr r0, _081D628C\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D62BA\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r5, r0]\n\t"
        "	cmp r1, #0x40\n\t"
        "	bne _081D6290\n\t"
        "	bl sub_081D6D74\n\t"
        "	b _081D62B4\n\t"
        "	.align 2, 0\n\t"
        "_081D6288: .4byte gUnknown_3005B68\n\t"
        "_081D628C: .4byte gPaletteFade\n\t"
        "_081D6290:\n\t"
        "	cmp r1, #0x90\n\t"
        "	bne _081D629A\n\t"
        "	bl sub_081D6DBC\n\t"
        "	b _081D62B4\n\t"
        "_081D629A:\n\t"
        "	cmp r1, #0x94\n\t"
        "	beq _081D62B0\n\t"
        "	movs r0, #0xa4\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D62B4\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #0\n\t"
        "	bl DuoFightEnd\n\t"
        "	b _081D62BA\n\t"
        "_081D62B0:\n\t"
        "	bl sub_081D6E04\n\t"
        "_081D62B4:\n\t"
        "	ldrh r0, [r5]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5]\n\t"
        "_081D62BA:\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D62C0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	ldr r0, _081D6348\n\t"
        "	mov r8, r0\n\t"
        "	movs r1, #0x58\n\t"
        "	movs r2, #0x48\n\t"
        "	movs r3, #3\n\t"
        "	bl CreateSprite\n\t"
        "	adds r5, r0, #0\n\t"
        "	lsls r5, r5, #0x18\n\t"
        "	lsrs r5, r5, #0x18\n\t"
        "	ldr r6, _081D634C\n\t"
        "	lsls r4, r5, #4\n\t"
        "	adds r4, r4, r5\n\t"
        "	lsls r4, r4, #2\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x1c\n\t"
        "	adds r0, r4, r0\n\t"
        "	ldr r1, _081D6350\n\t"
        "	str r1, [r0]\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x2e\n\t"
        "	adds r4, r4, r0\n\t"
        "	mov r0, r8\n\t"
        "	movs r1, #0x38\n\t"
        "	movs r2, #0x68\n\t"
        "	movs r3, #3\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4]\n\t"
        "	ldr r0, _081D6354\n\t"
        "	movs r1, #0x4b\n\t"
        "	movs r2, #0x65\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	ldr r0, _081D6358\n\t"
        "	movs r1, #0x6d\n\t"
        "	movs r2, #0x72\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	adds r0, r5, #0\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r1}\n\t"
        "	bx r1\n\t"
        "	.align 2, 0\n\t"
        "_081D6348: .4byte sSpriteTemplate_DuoFightPre_Groudon\n\t"
        "_081D634C: .4byte gSprites\n\t"
        "_081D6350: .4byte sub_081D635C + 1\n\t"
        "_081D6354: .4byte sSpriteTemplate_DuoFightPre_GroudonShoulder\n\t"
        "_081D6358: .4byte sSpriteTemplate_DuoFightPre_GroudonClaw\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D635C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	adds r3, r0, #0\n\t"
        "	movs r0, #0x2e\n\t"
        "	adds r0, r0, r3\n\t"
        "	mov ip, r0\n\t"
        "	ldrh r0, [r0, #0xa]\n\t"
        "	adds r0, #1\n\t"
        "	movs r1, #0x1f\n\t"
        "	ands r0, r1\n\t"
        "	mov r1, ip\n\t"
        "	strh r0, [r1, #0xa]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D63C0\n\t"
        "	ldrh r1, [r3, #0x20]\n\t"
        "	movs r2, #0x20\n\t"
        "	ldrsh r0, [r3, r2]\n\t"
        "	cmp r0, #0x48\n\t"
        "	beq _081D63C0\n\t"
        "	subs r0, r1, #1\n\t"
        "	strh r0, [r3, #0x20]\n\t"
        "	ldr r2, _081D63D4\n\t"
        "	movs r4, #0x2e\n\t"
        "	ldrsh r1, [r3, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "_081D63C0:\n\t"
        "	adds r0, r3, #0\n\t"
        "	adds r0, #0x2b\n\t"
        "	ldrb r3, [r0]\n\t"
        "	cmp r3, #1\n\t"
        "	beq _081D6420\n\t"
        "	cmp r3, #1\n\t"
        "	bgt _081D63D8\n\t"
        "	cmp r3, #0\n\t"
        "	beq _081D63E2\n\t"
        "	b _081D64A4\n\t"
        "	.align 2, 0\n\t"
        "_081D63D4: .4byte gSprites\n\t"
        "_081D63D8:\n\t"
        "	cmp r3, #2\n\t"
        "	beq _081D6460\n\t"
        "	cmp r3, #3\n\t"
        "	beq _081D6420\n\t"
        "	b _081D64A4\n\t"
        "_081D63E2:\n\t"
        "	ldr r2, _081D641C\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	b _081D649A\n\t"
        "	.align 2, 0\n\t"
        "_081D641C: .4byte gSprites\n\t"
        "_081D6420:\n\t"
        "	ldr r2, _081D6458\n\t"
        "	mov r0, ip\n\t"
        "	movs r3, #2\n\t"
        "	ldrsh r1, [r0, r3]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D645C\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	mov r4, ip\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r3, #0\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r1, #1\n\t"
        "	rsbs r1, r1, #0\n\t"
        "	b _081D6494\n\t"
        "	.align 2, 0\n\t"
        "_081D6458: .4byte gSprites\n\t"
        "_081D645C: .4byte 0x0000FFFF\n\t"
        "_081D6460:\n\t"
        "	ldr r2, _081D64AC\n\t"
        "	mov r3, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r3, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D64B0\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r3, #1\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	mov r4, ip\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D64B4\n\t"
        "_081D6494:\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "_081D649A:\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "_081D64A4:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D64AC: .4byte gSprites\n\t"
        "_081D64B0: .4byte 0x0000FFFF\n\t"
        "_081D64B4: .4byte 0x0000FFFE\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D64B8(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	ldr r5, _081D6648\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x88\n\t"
        "	movs r2, #0x60\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	mov r8, r0\n\t"
        "	mov r0, r8\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	mov r8, r0\n\t"
        "	ldr r6, _081D664C\n\t"
        "	lsls r4, r0, #4\n\t"
        "	add r4, r8\n\t"
        "	lsls r4, r4, #2\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x1c\n\t"
        "	adds r0, r4, r0\n\t"
        "	ldr r1, _081D6650\n\t"
        "	str r1, [r0]\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x2e\n\t"
        "	adds r4, r4, r0\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xa8\n\t"
        "	movs r2, #0x60\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x88\n\t"
        "	movs r2, #0x70\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xa8\n\t"
        "	movs r2, #0x70\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x88\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xa8\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x68\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #2\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #4]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x88\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #2\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xb8\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	ldr r0, _081D6654\n\t"
        "	movs r1, #0xd0\n\t"
        "	movs r2, #0x84\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	ldr r0, _081D6658\n\t"
        "	movs r1, #0xc8\n\t"
        "	movs r2, #0x78\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #8]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	ldrh r1, [r4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #2\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #3\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #4\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #5\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #6\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #7\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #6]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #8\n\t"
        "	bl StartSpriteAnim\n\t"
        "	mov r0, r8\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r1}\n\t"
        "	bx r1\n\t"
        "	.align 2, 0\n\t"
        "_081D6648: .4byte sSpriteTemplate_DuoFightPre_Kyogre\n\t"
        "_081D664C: .4byte gSprites\n\t"
        "_081D6650: .4byte sub_081D665C + 1\n\t"
        "_081D6654: .4byte sSpriteTemplate_DuoFightPre_KyogrePectoralFin\n\t"
        "_081D6658: .4byte sSpriteTemplate_DuoFightPre_KyogreDorsalFin\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D665C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	adds r4, r0, #0\n\t"
        "	adds r3, r4, #0\n\t"
        "	adds r3, #0x2e\n\t"
        "	ldrh r0, [r3, #0xa]\n\t"
        "	adds r0, #1\n\t"
        "	movs r1, #0x1f\n\t"
        "	ands r0, r1\n\t"
        "	strh r0, [r3, #0xa]\n\t"
        "	ldr r7, _081D676C\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D674A\n\t"
        "	ldrh r1, [r4, #0x20]\n\t"
        "	movs r2, #0x20\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x98\n\t"
        "	beq _081D674A\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #0x20]\n\t"
        "	ldrh r1, [r4, #0x2e]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r4, #0x2e]\n\t"
        "	movs r2, #0xff\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #8]\n\t"
        "	ands r2, r0\n\t"
        "	lsls r0, r2, #4\n\t"
        "	adds r0, r0, r2\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "_081D674A:\n\t"
        "	adds r6, r7, #0\n\t"
        "	movs r5, #0xff\n\t"
        "	ldrb r1, [r3, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	adds r0, #0x2b\n\t"
        "	ldrb r2, [r0]\n\t"
        "	cmp r2, #1\n\t"
        "	beq _081D681C\n\t"
        "	cmp r2, #1\n\t"
        "	bgt _081D6770\n\t"
        "	cmp r2, #0\n\t"
        "	beq _081D677C\n\t"
        "	b _081D6904\n\t"
        "	.align 2, 0\n\t"
        "_081D676C: .4byte gSprites\n\t"
        "_081D6770:\n\t"
        "	cmp r2, #2\n\t"
        "	bne _081D6776\n\t"
        "	b _081D68A4\n\t"
        "_081D6776:\n\t"
        "	cmp r2, #3\n\t"
        "	beq _081D681C\n\t"
        "	b _081D6904\n\t"
        "_081D677C:\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	ands r1, r5\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	b _081D6902\n\t"
        "_081D681C:\n\t"
        "	movs r2, #1\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #6]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #8]\n\t"
        "	b _081D68FA\n\t"
        "_081D68A4:\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	ands r1, r5\n\t"
        "_081D68FA:\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "_081D6902:\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "_081D6904:\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void VBlankCB_DuoFight(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	bl VBlankCB_RayquazaScene\n\t"
        "	bl ScanlineEffect_InitHBlankDmaTransfer\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D691C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	bl ResetVramOamAndBgCntRegs\n\t"
        "	movs r0, #0\n\t"
        "	bl ResetBgsAndClearDma3BusyFlags\n\t"
        "	ldr r1, _081D6998\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #3\n\t"
        "	bl InitBgsFromTemplates\n\t"
        "	ldr r4, _081D699C\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D69A0\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #1\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D69A4\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #2\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	bl ResetAllBgsCoordinates\n\t"
        "	movs r0, #0\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #1\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #2\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r1, #0x82\n\t"
        "	lsls r1, r1, #5\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #1\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #2\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #0x50\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6998: .4byte sBgTemplates_DuoFight\n\t"
        "_081D699C: .4byte gUnknown_203CC2C\n\t"
        "_081D69A0: .4byte 0x00000804\n\t"
        "_081D69A4: .4byte 0x00001004\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D69A8(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #4\n\t"
        "	bl ResetTempTileDataBuffers\n\t"
        "	ldr r1, _081D6A30\n\t"
        "	movs r0, #0\n\t"
        "	str r0, [sp]\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "_081D69BE:\n\t"
        "	bl FreeTempTileDataBuffersIfPossible\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D69BE\n\t"
        "	ldr r0, _081D6A34\n\t"
        "	ldr r4, _081D6A38\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D6A3C\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D6A40\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D6A44\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D6A48\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D6A4C\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x40\n\t"
        "	bl LoadCompressedPalette\n\t"
        "	ldr r0, _081D6A50\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A54\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A58\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A5C\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A60\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A64\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D6A68\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	ldr r0, _081D6A6C\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	add sp, #4\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6A30: .4byte gRaySceneDuoFight_Clouds_Gfx\n\t"
        "_081D6A34: .4byte gRaySceneDuoFight_Clouds2_Tilemap\n\t"
        "_081D6A38: .4byte gUnknown_203CC2C\n\t"
        "_081D6A3C: .4byte gRaySceneDuoFight_Clouds1_Tilemap\n\t"
        "_081D6A40: .4byte 0x00000804\n\t"
        "_081D6A44: .4byte gRaySceneDuoFight_Clouds3_Tilemap\n\t"
        "_081D6A48: .4byte 0x00001004\n\t"
        "_081D6A4C: .4byte gRaySceneDuoFight_Clouds_Pal\n\t"
        "_081D6A50: .4byte sSpriteSheet_DuoFight_Groudon\n\t"
        "_081D6A54: .4byte sSpriteSheet_DuoFight_GroudonShoulder\n\t"
        "_081D6A58: .4byte sSpriteSheet_DuoFight_GroudonClaw\n\t"
        "_081D6A5C: .4byte sSpriteSheet_DuoFight_Kyogre\n\t"
        "_081D6A60: .4byte sSpriteSheet_DuoFight_KyogrePectoralFin\n\t"
        "_081D6A64: .4byte sSpriteSheet_DuoFight_KyogreDorsalFin\n\t"
        "_081D6A68: .4byte sSpritePal_DuoFight_Groudon\n\t"
        "_081D6A6C: .4byte sSpritePal_DuoFight_Kyogre\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_DuoFightAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	sub sp, #8\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r6, r1, #3\n\t"
        "	ldr r7, _081D6AE8\n\t"
        "	adds r5, r6, r7\n\t"
        "	bl ScanlineEffect_Clear\n\t"
        "	bl sub_081D691C\n\t"
        "	bl sub_081D69A8\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp, #4]\n\t"
        "	ldr r1, _081D6AEC\n\t"
        "	ldr r2, _081D6AF0\n\t"
        "	add r0, sp, #4\n\t"
        "	bl CpuFastSet\n\t"
        "	ldr r2, _081D6AF4\n\t"
        "	ldr r0, [r2]\n\t"
        "	ldr r1, [r2, #4]\n\t"
        "	ldr r2, [r2, #8]\n\t"
        "	bl ScanlineEffect_SetParams\n\t"
        "	strh r4, [r5]\n\t"
        "	ldr r0, _081D6AF8\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r5, #2]\n\t"
        "	ldr r0, _081D6AFC\n\t"
        "	ldr r0, [r0]\n\t"
        "	ldr r1, _081D6B00\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldrb r0, [r0]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D6B08\n\t"
        "	bl sub_081D62C0\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r5, #4]\n\t"
        "	bl sub_081D64B8\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r5, #6]\n\t"
        "	adds r0, r7, #0\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r6, r0\n\t"
        "	ldr r1, _081D6B04\n\t"
        "	str r1, [r0]\n\t"
        "	b _081D6B2A\n\t"
        "	.align 2, 0\n\t"
        "_081D6AE8: .4byte gUnknown_3005B68\n\t"
        "_081D6AEC: .4byte gScanlineEffectRegBuffers\n\t"
        "_081D6AF0: .4byte 0x010003C0\n\t"
        "_081D6AF4: .4byte sScanlineParams_DuoFight_Clouds\n\t"
        "_081D6AF8: .4byte sub_081D6B64 + 1\n\t"
        "_081D6AFC: .4byte gUnknown_203CC2C\n\t"
        "_081D6B00: .4byte 0x00002006\n\t"
        "_081D6B04: .4byte Task_HandleDuoFightPre + 1\n\t"
        "_081D6B08:\n\t"
        "	bl sub_081D6FA0\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r5, #4]\n\t"
        "	bl sub_081D71F8\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r5, #6]\n\t"
        "	adds r0, r7, #0\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r6, r0\n\t"
        "	ldr r1, _081D6B5C\n\t"
        "	str r1, [r0]\n\t"
        "	bl StopMapMusic\n\t"
        "_081D6B2A:\n\t"
        "	movs r4, #1\n\t"
        "	rsbs r4, r4, #0\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0\n\t"
        "	bl BlendPalettes\n\t"
        "	movs r0, #0\n\t"
        "	str r0, [sp]\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	ldr r0, _081D6B60\n\t"
        "	bl SetVBlankCallback\n\t"
        "	movs r0, #0x53\n\t"
        "	bl PlaySE\n\t"
        "	add sp, #8\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6B5C: .4byte Task_HandleDuoFight + 1\n\t"
        "_081D6B60: .4byte VBlankCB_DuoFight + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6B64(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D6B9C\n\t"
        "	adds r3, r1, r0\n\t"
        "	movs r1, #0x18\n\t"
        "	ldr r7, _081D6BA0\n\t"
        "	ldr r4, _081D6BA4\n\t"
        "	movs r0, #0xf0\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r5, r4, r0\n\t"
        "_081D6B80:\n\t"
        "	lsls r0, r1, #0x10\n\t"
        "	asrs r1, r0, #0x10\n\t"
        "	adds r6, r0, #0\n\t"
        "	cmp r1, #0x2f\n\t"
        "	bgt _081D6BA8\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3]\n\t"
        "	b _081D6C06\n\t"
        "	.align 2, 0\n\t"
        "_081D6B9C: .4byte gUnknown_3005B68\n\t"
        "_081D6BA0: .4byte gUnknown_203CC2C\n\t"
        "_081D6BA4: .4byte gScanlineEffectRegBuffers\n\t"
        "_081D6BA8:\n\t"
        "	cmp r1, #0x3f\n\t"
        "	bgt _081D6BBC\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	b _081D6C06\n\t"
        "_081D6BBC:\n\t"
        "	cmp r1, #0x4b\n\t"
        "	bgt _081D6BD0\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	b _081D6C06\n\t"
        "_081D6BD0:\n\t"
        "	cmp r1, #0x53\n\t"
        "	bgt _081D6BE4\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	b _081D6C06\n\t"
        "_081D6BE4:\n\t"
        "	cmp r1, #0x57\n\t"
        "	bgt _081D6BF8\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3, #8]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3, #8]\n\t"
        "	b _081D6C06\n\t"
        "_081D6BF8:\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r2, r1, r4\n\t"
        "	ldrh r0, [r3, #0xa]\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r2]\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrh r0, [r3, #0xa]\n\t"
        "_081D6C06:\n\t"
        "	lsrs r0, r0, #8\n\t"
        "	strh r0, [r1]\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #9\n\t"
        "	adds r0, r6, r1\n\t"
        "	lsrs r1, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	cmp r0, #0x5b\n\t"
        "	ble _081D6B80\n\t"
        "	ldr r0, [r7]\n\t"
        "	ldr r1, _081D6C5C\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldrb r0, [r0]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D6C60\n\t"
        "	movs r1, #0xe0\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3]\n\t"
        "	movs r1, #0xc0\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #2]\n\t"
        "	movs r1, #0xa0\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #4]\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #6]\n\t"
        "	ldrh r0, [r3, #8]\n\t"
        "	adds r0, #0xc0\n\t"
        "	b _081D6C9A\n\t"
        "	.align 2, 0\n\t"
        "_081D6C5C: .4byte 0x00002006\n\t"
        "_081D6C60:\n\t"
        "	movs r1, #0xc0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3]\n\t"
        "	movs r1, #0xa0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #2]\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #2\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #4]\n\t"
        "	movs r1, #0xc0\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r3, #6]\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #1\n\t"
        "	adds r0, r1, #0\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	adds r0, r0, r1\n\t"
        "_081D6C9A:\n\t"
        "	strh r0, [r3, #8]\n\t"
        "	ldrh r0, [r3, #0xa]\n\t"
        "	adds r0, #0x80\n\t"
        "	strh r0, [r3, #0xa]\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleDuoFight(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	lsls r0, r5, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r7, _081D6CE0\n\t"
        "	adds r4, r0, r7\n\t"
        "	bl sub_081D6E48\n\t"
        "	ldr r0, _081D6CE4\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r6, r0, #0x18\n\t"
        "	cmp r6, #0\n\t"
        "	bne _081D6D6A\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	cmp r1, #0x20\n\t"
        "	beq _081D6CD8\n\t"
        "	cmp r1, #0x70\n\t"
        "	bne _081D6CE8\n\t"
        "_081D6CD8:\n\t"
        "	bl sub_081D6D74\n\t"
        "	b _081D6D64\n\t"
        "	.align 2, 0\n\t"
        "_081D6CE0: .4byte gUnknown_3005B68\n\t"
        "_081D6CE4: .4byte gPaletteFade\n\t"
        "_081D6CE8:\n\t"
        "	cmp r1, #0xd8\n\t"
        "	bne _081D6CF2\n\t"
        "	bl sub_081D6DBC\n\t"
        "	b _081D6D64\n\t"
        "_081D6CF2:\n\t"
        "	cmp r1, #0xdc\n\t"
        "	bne _081D6CFC\n\t"
        "	bl sub_081D6E04\n\t"
        "	b _081D6D64\n\t"
        "_081D6CFC:\n\t"
        "	movs r0, #0xbe\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	beq _081D6D14\n\t"
        "	adds r0, #0x20\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D6D64\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #2\n\t"
        "	bl DuoFightEnd\n\t"
        "	b _081D6D6A\n\t"
        "_081D6D14:\n\t"
        "	movs r1, #0x91\n\t"
        "	lsls r1, r1, #2\n\t"
        "	movs r0, #0x50\n\t"
        "	bl SetGpuReg\n\t"
        "	adds r2, r7, #0\n\t"
        "	subs r2, #8\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D6D70\n\t"
        "	str r1, [r0]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r6, [r0, #8]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r4, #4]\n\t"
        "	strh r1, [r0, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	strh r1, [r0, #0xe]\n\t"
        "	bl ScanlineEffect_Stop\n\t"
        "_081D6D64:\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "_081D6D6A:\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6D70: .4byte sub_081D6E68 + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6D74(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #0xc\n\t"
        "	movs r0, #0x57\n\t"
        "	bl PlaySE\n\t"
        "	ldr r0, _081D6DB0\n\t"
        "	ldr r1, _081D6DB4\n\t"
        "	str r1, [sp]\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp, #4]\n\t"
        "	str r4, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	ldr r0, _081D6DB8\n\t"
        "	str r4, [sp]\n\t"
        "	str r4, [sp, #4]\n\t"
        "	movs r1, #1\n\t"
        "	str r1, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	add sp, #0xc\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6DB0: .4byte 0x00007FFF\n\t"
        "_081D6DB4: .4byte 0x0000FFFF\n\t"
        "_081D6DB8: .4byte 0xFFFF0000\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6DBC(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #0xc\n\t"
        "	movs r0, #0x57\n\t"
        "	bl PlaySE\n\t"
        "	ldr r0, _081D6DF8\n\t"
        "	ldr r1, _081D6DFC\n\t"
        "	str r1, [sp]\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp, #4]\n\t"
        "	str r4, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0x10\n\t"
        "	bl sub_080A250C\n\t"
        "	ldr r0, _081D6E00\n\t"
        "	str r4, [sp]\n\t"
        "	str r4, [sp, #4]\n\t"
        "	movs r1, #1\n\t"
        "	str r1, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0x10\n\t"
        "	bl sub_080A250C\n\t"
        "	add sp, #0xc\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6DF8: .4byte 0x00007FFF\n\t"
        "_081D6DFC: .4byte 0x0000FFFF\n\t"
        "_081D6E00: .4byte 0xFFFF0000\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6E04(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #0xc\n\t"
        "	ldr r0, _081D6E3C\n\t"
        "	ldr r1, _081D6E40\n\t"
        "	str r1, [sp]\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp, #4]\n\t"
        "	str r4, [sp, #8]\n\t"
        "	movs r1, #4\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	ldr r0, _081D6E44\n\t"
        "	str r4, [sp]\n\t"
        "	str r4, [sp, #4]\n\t"
        "	movs r1, #1\n\t"
        "	str r1, [sp, #8]\n\t"
        "	movs r1, #4\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	add sp, #0xc\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6E3C: .4byte 0x00007FFF\n\t"
        "_081D6E40: .4byte 0x0000FFFF\n\t"
        "_081D6E44: .4byte 0xFFFF0000\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6E48(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #3\n\t"
        "	movs r0, #2\n\t"
        "	movs r2, #1\n\t"
        "	bl ChangeBgX\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #4\n\t"
        "	movs r0, #2\n\t"
        "	movs r2, #2\n\t"
        "	bl ChangeBgY\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6E68(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D6EE8\n\t"
        "	adds r5, r1, r0\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r5, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	ldr r4, _081D6EEC\n\t"
        "	adds r0, r0, r4\n\t"
        "	bl sub_081D719C\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r5, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r4\n\t"
        "	bl sub_081D7650\n\t"
        "	movs r0, #1\n\t"
        "	bl GetBgY\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	lsrs r4, r0, #0x10\n\t"
        "	movs r0, #1\n\t"
        "	bl GetBgY\n\t"
        "	cmp r0, #0\n\t"
        "	beq _081D6EB6\n\t"
        "	movs r0, #0x80\n\t"
        "	lsls r0, r0, #8\n\t"
        "	cmp r4, r0\n\t"
        "	bls _081D6EC2\n\t"
        "_081D6EB6:\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #3\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #2\n\t"
        "	bl ChangeBgY\n\t"
        "_081D6EC2:\n\t"
        "	ldrh r2, [r5]\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r5, r1]\n\t"
        "	cmp r0, #0x10\n\t"
        "	beq _081D6EE2\n\t"
        "	adds r2, #1\n\t"
        "	strh r2, [r5]\n\t"
        "	lsls r1, r2, #8\n\t"
        "	movs r0, #0x10\n\t"
        "	subs r0, r0, r2\n\t"
        "	orrs r1, r0\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	lsrs r1, r1, #0x10\n\t"
        "	movs r0, #0x52\n\t"
        "	bl SetGpuReg\n\t"
        "_081D6EE2:\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6EE8: .4byte gUnknown_3005B68\n\t"
        "_081D6EEC: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void DuoFightEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	sub sp, #4\n\t"
        "	adds r5, r0, #0\n\t"
        "	adds r4, r1, #0\n\t"
        "	lsls r5, r5, #0x18\n\t"
        "	lsrs r5, r5, #0x18\n\t"
        "	lsls r4, r4, #0x18\n\t"
        "	lsrs r4, r4, #0x18\n\t"
        "	movs r0, #0x54\n\t"
        "	bl PlaySE\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	lsls r4, r4, #0x18\n\t"
        "	asrs r4, r4, #0x18\n\t"
        "	movs r1, #0\n\t"
        "	str r1, [sp]\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0x10\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	ldr r1, _081D6F34\n\t"
        "	lsls r0, r5, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r1, _081D6F38\n\t"
        "	str r1, [r0]\n\t"
        "	add sp, #4\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6F34: .4byte gTasks\n\t"
        "_081D6F38: .4byte Task_DuoFightEnd + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_DuoFightEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r6, r1, #3\n\t"
        "	ldr r7, _081D6F94\n\t"
        "	adds r5, r6, r7\n\t"
        "	bl sub_081D6E48\n\t"
        "	ldr r0, _081D6F98\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r4, r0, #0x18\n\t"
        "	cmp r4, #0\n\t"
        "	bne _081D6F8E\n\t"
        "	ldrb r0, [r5, #2]\n\t"
        "	bl DestroyTask\n\t"
        "	movs r0, #1\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0\n\t"
        "	bl ChangeBgY\n\t"
        "	movs r0, #0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	bl ScanlineEffect_Stop\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	strh r4, [r5]\n\t"
        "	adds r0, r7, #0\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r6, r0\n\t"
        "	ldr r1, _081D6F9C\n\t"
        "	str r1, [r0]\n\t"
        "_081D6F8E:\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D6F94: .4byte gUnknown_3005B68\n\t"
        "_081D6F98: .4byte gPaletteFade\n\t"
        "_081D6F9C: .4byte Task_SetNextAnim + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D6FA0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	ldr r0, _081D7028\n\t"
        "	mov r8, r0\n\t"
        "	movs r1, #0x62\n\t"
        "	movs r2, #0x48\n\t"
        "	movs r3, #3\n\t"
        "	bl CreateSprite\n\t"
        "	adds r5, r0, #0\n\t"
        "	lsls r5, r5, #0x18\n\t"
        "	lsrs r5, r5, #0x18\n\t"
        "	ldr r6, _081D702C\n\t"
        "	lsls r4, r5, #4\n\t"
        "	adds r4, r4, r5\n\t"
        "	lsls r4, r4, #2\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x1c\n\t"
        "	adds r0, r4, r0\n\t"
        "	ldr r1, _081D7030\n\t"
        "	str r1, [r0]\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x2e\n\t"
        "	adds r4, r4, r0\n\t"
        "	mov r0, r8\n\t"
        "	movs r1, #0x42\n\t"
        "	movs r2, #0x68\n\t"
        "	movs r3, #3\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4]\n\t"
        "	ldr r0, _081D7034\n\t"
        "	movs r1, #0x55\n\t"
        "	movs r2, #0x65\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	ldr r0, _081D7038\n\t"
        "	movs r1, #0x77\n\t"
        "	movs r2, #0x72\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	adds r0, r5, #0\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r1}\n\t"
        "	bx r1\n\t"
        "	.align 2, 0\n\t"
        "_081D7028: .4byte sSpriteTemplate_DuoFight_Groudon\n\t"
        "_081D702C: .4byte gSprites\n\t"
        "_081D7030: .4byte sub_081D703C + 1\n\t"
        "_081D7034: .4byte sSpriteTemplate_DuoFight_GroudonShoulder\n\t"
        "_081D7038: .4byte sSpriteTemplate_DuoFight_GroudonClaw\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D703C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	adds r3, r0, #0\n\t"
        "	movs r0, #0x2e\n\t"
        "	adds r0, r0, r3\n\t"
        "	mov ip, r0\n\t"
        "	ldrh r0, [r0, #0xa]\n\t"
        "	adds r0, #1\n\t"
        "	movs r1, #0xf\n\t"
        "	ands r0, r1\n\t"
        "	mov r1, ip\n\t"
        "	strh r0, [r1, #0xa]\n\t"
        "	movs r1, #7\n\t"
        "	ands r1, r0\n\t"
        "	cmp r1, #0\n\t"
        "	bne _081D70A4\n\t"
        "	ldrh r1, [r3, #0x20]\n\t"
        "	movs r2, #0x20\n\t"
        "	ldrsh r0, [r3, r2]\n\t"
        "	cmp r0, #0x48\n\t"
        "	beq _081D70A4\n\t"
        "	subs r0, r1, #1\n\t"
        "	strh r0, [r3, #0x20]\n\t"
        "	ldr r2, _081D70B8\n\t"
        "	movs r4, #0x2e\n\t"
        "	ldrsh r1, [r3, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "_081D70A4:\n\t"
        "	adds r0, r3, #0\n\t"
        "	adds r0, #0x2b\n\t"
        "	ldrb r3, [r0]\n\t"
        "	cmp r3, #1\n\t"
        "	beq _081D7104\n\t"
        "	cmp r3, #1\n\t"
        "	bgt _081D70BC\n\t"
        "	cmp r3, #0\n\t"
        "	beq _081D70C6\n\t"
        "	b _081D7188\n\t"
        "	.align 2, 0\n\t"
        "_081D70B8: .4byte gSprites\n\t"
        "_081D70BC:\n\t"
        "	cmp r3, #2\n\t"
        "	beq _081D7144\n\t"
        "	cmp r3, #3\n\t"
        "	beq _081D7104\n\t"
        "	b _081D7188\n\t"
        "_081D70C6:\n\t"
        "	ldr r2, _081D7100\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	mov r0, ip\n\t"
        "	movs r4, #4\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	b _081D717E\n\t"
        "	.align 2, 0\n\t"
        "_081D7100: .4byte gSprites\n\t"
        "_081D7104:\n\t"
        "	ldr r2, _081D713C\n\t"
        "	mov r0, ip\n\t"
        "	movs r3, #2\n\t"
        "	ldrsh r1, [r0, r3]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D7140\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	mov r4, ip\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r3, #0\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r1, #1\n\t"
        "	rsbs r1, r1, #0\n\t"
        "	b _081D7178\n\t"
        "	.align 2, 0\n\t"
        "_081D713C: .4byte gSprites\n\t"
        "_081D7140: .4byte 0x0000FFFF\n\t"
        "_081D7144:\n\t"
        "	ldr r2, _081D7190\n\t"
        "	mov r3, ip\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r3, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D7194\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r3, #1\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "	mov r4, ip\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D7198\n\t"
        "_081D7178:\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "_081D717E:\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x26]\n\t"
        "_081D7188:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7190: .4byte gSprites\n\t"
        "_081D7194: .4byte 0x0000FFFF\n\t"
        "_081D7198: .4byte 0x0000FFFE\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D719C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	adds r1, r0, #0\n\t"
        "	adds r3, r1, #0\n\t"
        "	adds r3, #0x2e\n\t"
        "	ldrh r2, [r1, #0x22]\n\t"
        "	movs r4, #0x22\n\t"
        "	ldrsh r0, [r1, r4]\n\t"
        "	cmp r0, #0xa0\n\t"
        "	bgt _081D71EC\n\t"
        "	adds r0, r2, #0\n\t"
        "	adds r0, #8\n\t"
        "	strh r0, [r1, #0x22]\n\t"
        "	ldr r2, _081D71F4\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r1, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	movs r4, #2\n\t"
        "	ldrsh r1, [r3, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "_081D71EC:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D71F4: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D71F8(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	ldr r5, _081D7388\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x7e\n\t"
        "	movs r2, #0x60\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	mov r8, r0\n\t"
        "	mov r0, r8\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	mov r8, r0\n\t"
        "	ldr r6, _081D738C\n\t"
        "	lsls r4, r0, #4\n\t"
        "	add r4, r8\n\t"
        "	lsls r4, r4, #2\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x1c\n\t"
        "	adds r0, r4, r0\n\t"
        "	ldr r1, _081D7390\n\t"
        "	str r1, [r0]\n\t"
        "	adds r0, r6, #0\n\t"
        "	adds r0, #0x2e\n\t"
        "	adds r4, r4, r0\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x9e\n\t"
        "	movs r2, #0x60\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x7e\n\t"
        "	movs r2, #0x70\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x9e\n\t"
        "	movs r2, #0x70\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x7e\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x9e\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x5e\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #2\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #4]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0x7e\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #2\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xae\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	ldr r0, _081D7394\n\t"
        "	movs r1, #0xc6\n\t"
        "	movs r2, #0x84\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #8\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	ldr r0, _081D7398\n\t"
        "	movs r1, #0xbe\n\t"
        "	movs r2, #0x78\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r4, #8]\n\t"
        "	orrs r0, r1\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	ldrh r1, [r4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #2\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #3\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #4\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #5\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #6\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #7\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldrb r1, [r4, #6]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	movs r1, #8\n\t"
        "	bl StartSpriteAnim\n\t"
        "	mov r0, r8\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r1}\n\t"
        "	bx r1\n\t"
        "	.align 2, 0\n\t"
        "_081D7388: .4byte sSpriteTemplate_DuoFight_Kyogre\n\t"
        "_081D738C: .4byte gSprites\n\t"
        "_081D7390: .4byte sub_081D739C + 1\n\t"
        "_081D7394: .4byte sSpriteTemplate_DuoFight_KyogrePectoralFin\n\t"
        "_081D7398: .4byte sSpriteTemplate_DuoFight_KyogreDorsalFin\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D739C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	adds r4, r0, #0\n\t"
        "	adds r3, r4, #0\n\t"
        "	adds r3, #0x2e\n\t"
        "	ldrh r0, [r3, #0xa]\n\t"
        "	adds r0, #1\n\t"
        "	movs r1, #0xf\n\t"
        "	ands r0, r1\n\t"
        "	strh r0, [r3, #0xa]\n\t"
        "	movs r1, #7\n\t"
        "	ands r1, r0\n\t"
        "	ldr r7, _081D74B0\n\t"
        "	cmp r1, #0\n\t"
        "	bne _081D748E\n\t"
        "	ldrh r1, [r4, #0x20]\n\t"
        "	movs r2, #0x20\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x98\n\t"
        "	beq _081D748E\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #0x20]\n\t"
        "	ldrh r1, [r4, #0x2e]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r4, #0x2e]\n\t"
        "	movs r2, #0xff\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r0, [r3, #8]\n\t"
        "	ands r2, r0\n\t"
        "	lsls r0, r2, #4\n\t"
        "	adds r0, r0, r2\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "_081D748E:\n\t"
        "	adds r6, r7, #0\n\t"
        "	movs r5, #0xff\n\t"
        "	ldrb r1, [r3, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	adds r0, #0x2b\n\t"
        "	ldrb r2, [r0]\n\t"
        "	cmp r2, #1\n\t"
        "	beq _081D7560\n\t"
        "	cmp r2, #1\n\t"
        "	bgt _081D74B4\n\t"
        "	cmp r2, #0\n\t"
        "	beq _081D74C0\n\t"
        "	b _081D7648\n\t"
        "	.align 2, 0\n\t"
        "_081D74B0: .4byte gSprites\n\t"
        "_081D74B4:\n\t"
        "	cmp r2, #2\n\t"
        "	bne _081D74BA\n\t"
        "	b _081D75E8\n\t"
        "_081D74BA:\n\t"
        "	cmp r2, #3\n\t"
        "	beq _081D7560\n\t"
        "	b _081D7648\n\t"
        "_081D74C0:\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #4]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #6]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	ands r1, r5\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	b _081D7646\n\t"
        "_081D7560:\n\t"
        "	movs r2, #1\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #6]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrb r1, [r3, #8]\n\t"
        "	b _081D763E\n\t"
        "_081D75E8:\n\t"
        "	strh r2, [r4, #0x26]\n\t"
        "	ldrh r1, [r3]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #2]\n\t"
        "	adds r1, r5, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r1, [r3, #8]\n\t"
        "	ands r1, r5\n\t"
        "_081D763E:\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r7\n\t"
        "_081D7646:\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "_081D7648:\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D7650(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	adds r2, r0, #0\n\t"
        "	movs r0, #0x2e\n\t"
        "	adds r0, r0, r2\n\t"
        "	mov ip, r0\n\t"
        "	ldrh r1, [r2, #0x22]\n\t"
        "	movs r3, #0x22\n\t"
        "	ldrsh r0, [r2, r3]\n\t"
        "	cmp r0, #0xa0\n\t"
        "	bgt _081D7744\n\t"
        "	adds r0, r1, #0\n\t"
        "	adds r0, #8\n\t"
        "	strh r0, [r2, #0x22]\n\t"
        "	ldr r3, _081D7748\n\t"
        "	ldrh r1, [r2, #0x2e]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	ldrh r0, [r2, #0x2e]\n\t"
        "	movs r2, #0xff\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r0, ip\n\t"
        "	ldrh r1, [r0, #2]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r1, ip\n\t"
        "	ldrh r0, [r1, #2]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r0, ip\n\t"
        "	ldrh r1, [r0, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r1, ip\n\t"
        "	ldrh r0, [r1, #4]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r0, ip\n\t"
        "	ldrh r1, [r0, #6]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r1, ip\n\t"
        "	ldrh r0, [r1, #6]\n\t"
        "	adds r1, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r0, ip\n\t"
        "	ldrh r1, [r0, #8]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "	mov r1, ip\n\t"
        "	ldrh r0, [r1, #8]\n\t"
        "	ands r2, r0\n\t"
        "	lsls r0, r2, #4\n\t"
        "	adds r0, r0, r2\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #8\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "_081D7744:\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7748: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D774C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	bl ResetVramOamAndBgCntRegs\n\t"
        "	movs r0, #0\n\t"
        "	bl ResetBgsAndClearDma3BusyFlags\n\t"
        "	ldr r1, _081D77C8\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #3\n\t"
        "	bl InitBgsFromTemplates\n\t"
        "	ldr r4, _081D77CC\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D77D0\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #1\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D77D4\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #2\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	bl ResetAllBgsCoordinates\n\t"
        "	movs r0, #0\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #1\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #2\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r1, #0x82\n\t"
        "	lsls r1, r1, #5\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #1\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #2\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #0x50\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D77C8: .4byte sBgTemplates_TakesFlight\n\t"
        "_081D77CC: .4byte gUnknown_203CC2C\n\t"
        "_081D77D0: .4byte 0x00000804\n\t"
        "_081D77D4: .4byte 0x00001004\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D77D8(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #4\n\t"
        "	bl ResetTempTileDataBuffers\n\t"
        "	ldr r1, _081D7858\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D785C\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D7860\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #2\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "_081D780C:\n\t"
        "	bl FreeTempTileDataBuffersIfPossible\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D780C\n\t"
        "	ldr r0, _081D7864\n\t"
        "	ldr r4, _081D7868\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D786C\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D7870\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D7874\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D7878\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D787C\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x40\n\t"
        "	bl LoadCompressedPalette\n\t"
        "	ldr r0, _081D7880\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D7884\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	add sp, #4\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7858: .4byte gRaySceneDuoFight_Clouds_Gfx\n\t"
        "_081D785C: .4byte gRaySceneTakesFlight_Rayquaza_Gfx\n\t"
        "_081D7860: .4byte gRaySceneTakesFlight_Bg_Gfx\n\t"
        "_081D7864: .4byte gRaySceneDuoFight_Clouds2_Tilemap\n\t"
        "_081D7868: .4byte gUnknown_203CC2C\n\t"
        "_081D786C: .4byte gRaySceneTakesFlight_Rayquaza_Tilemap\n\t"
        "_081D7870: .4byte 0x00000804\n\t"
        "_081D7874: .4byte gRaySceneTakesFlight_Bg_Tilemap\n\t"
        "_081D7878: .4byte 0x00001004\n\t"
        "_081D787C: .4byte gRaySceneTakesFlight_Rayquaza_Pal\n\t"
        "_081D7880: .4byte sSpriteSheet_TakesFlight_Smoke\n\t"
        "_081D7884: .4byte sSpritePal_TakesFlight_Smoke\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayTakesFlightAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r4, r0, #2\n\t"
        "	adds r4, r4, r0\n\t"
        "	lsls r4, r4, #3\n\t"
        "	ldr r5, _081D78E8\n\t"
        "	adds r6, r4, r5\n\t"
        "	movs r0, #0xe8\n\t"
        "	lsls r0, r0, #1\n\t"
        "	bl PlayNewMapMusic\n\t"
        "	bl sub_081D774C\n\t"
        "	bl sub_081D77D8\n\t"
        "	movs r1, #0x94\n\t"
        "	lsls r1, r1, #2\n\t"
        "	movs r0, #0x50\n\t"
        "	bl SetGpuReg\n\t"
        "	ldr r1, _081D78EC\n\t"
        "	movs r0, #0x52\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0\n\t"
        "	bl BlendPalettes\n\t"
        "	ldr r0, _081D78F0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	ldr r0, _081D78F4\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r6]\n\t"
        "	strh r0, [r6, #2]\n\t"
        "	subs r5, #8\n\t"
        "	adds r4, r4, r5\n\t"
        "	ldr r0, _081D78F8\n\t"
        "	str r0, [r4]\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D78E8: .4byte gUnknown_3005B68\n\t"
        "_081D78EC: .4byte 0x00000808\n\t"
        "_081D78F0: .4byte VBlankCB_RayquazaScene + 1\n\t"
        "_081D78F4: .4byte sub_081D7AE0 + 1\n\t"
        "_081D78F8: .4byte Task_HandleRayTakesFlight + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleRayTakesFlight(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	sub sp, #0x10\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r6, r0, #0x18\n\t"
        "	lsls r0, r6, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D7920\n\t"
        "	adds r4, r0, r1\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r5, [r4, r0]\n\t"
        "	cmp r5, #1\n\t"
        "	beq _081D7966\n\t"
        "	cmp r5, #1\n\t"
        "	bgt _081D7924\n\t"
        "	cmp r5, #0\n\t"
        "	beq _081D7930\n\t"
        "	b _081D7A96\n\t"
        "	.align 2, 0\n\t"
        "_081D7920: .4byte gUnknown_3005B68\n\t"
        "_081D7924:\n\t"
        "	cmp r5, #2\n\t"
        "	beq _081D79E0\n\t"
        "	cmp r5, #3\n\t"
        "	bne _081D792E\n\t"
        "	b _081D7A60\n\t"
        "_081D792E:\n\t"
        "	b _081D7A96\n\t"
        "_081D7930:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #8\n\t"
        "	bne _081D7960\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	str r5, [sp]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	strh r5, [r4, #4]\n\t"
        "	movs r0, #0x1e\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	strh r5, [r4, #8]\n\t"
        "	movs r0, #7\n\t"
        "	strh r0, [r4, #0xa]\n\t"
        "	strh r5, [r4, #2]\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	b _081D7A96\n\t"
        "_081D7960:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	b _081D7A96\n\t"
        "_081D7966:\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	ldrh r3, [r4, #4]\n\t"
        "	adds r0, r1, r3\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	ldrh r2, [r4, #0xa]\n\t"
        "	ldrh r3, [r4, #8]\n\t"
        "	adds r0, r2, r3\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	lsls r0, r1, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	cmp r0, #3\n\t"
        "	ble _081D7982\n\t"
        "	subs r0, r1, #3\n\t"
        "	strh r0, [r4, #6]\n\t"
        "_081D7982:\n\t"
        "	lsls r0, r2, #0x10\n\t"
        "	cmp r0, #0\n\t"
        "	beq _081D798C\n\t"
        "	subs r0, r2, #1\n\t"
        "	strh r0, [r4, #0xa]\n\t"
        "_081D798C:\n\t"
        "	movs r1, #4\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0xff\n\t"
        "	ble _081D79B4\n\t"
        "	movs r2, #0x80\n\t"
        "	lsls r2, r2, #1\n\t"
        "	adds r0, r2, #0\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	movs r1, #0\n\t"
        "	strh r1, [r4, #6]\n\t"
        "	movs r0, #0xc\n\t"
        "	strh r0, [r4, #0xc]\n\t"
        "	movs r3, #1\n\t"
        "	rsbs r3, r3, #0\n\t"
        "	adds r0, r3, #0\n\t"
        "	strh r0, [r4, #0xe]\n\t"
        "	strh r1, [r4, #2]\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "_081D79B4:\n\t"
        "	movs r1, #0xf0\n\t"
        "	lsls r1, r1, #7\n\t"
        "	movs r2, #0xc0\n\t"
        "	lsls r2, r2, #5\n\t"
        "	ldrh r0, [r4, #8]\n\t"
        "	adds r0, #0x20\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	str r0, [sp]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #4]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #8]\n\t"
        "	movs r0, #0\n\t"
        "	str r0, [sp, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	movs r3, #0x78\n\t"
        "	bl SetBgAffine\n\t"
        "	b _081D7A96\n\t"
        "_081D79E0:\n\t"
        "	ldrh r0, [r4, #2]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	movs r1, #0xf0\n\t"
        "	lsls r1, r1, #7\n\t"
        "	movs r2, #0xc0\n\t"
        "	lsls r2, r2, #5\n\t"
        "	ldrh r0, [r4, #0xc]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x12\n\t"
        "	adds r0, #0x20\n\t"
        "	ldrh r3, [r4, #8]\n\t"
        "	adds r0, r0, r3\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	str r0, [sp]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #4]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #8]\n\t"
        "	movs r5, #0\n\t"
        "	str r5, [sp, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	movs r3, #0x78\n\t"
        "	bl SetBgAffine\n\t"
        "	ldrh r0, [r4, #0xe]\n\t"
        "	ldrh r1, [r4, #0xc]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r4, #0xc]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r1, r0, #0x10\n\t"
        "	cmp r1, #0xc\n\t"
        "	beq _081D7A30\n\t"
        "	movs r0, #0xc\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D7A96\n\t"
        "_081D7A30:\n\t"
        "	movs r2, #0xe\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	strh r0, [r4, #0xe]\n\t"
        "	movs r3, #2\n\t"
        "	ldrsh r1, [r4, r3]\n\t"
        "	ldr r0, _081D7A5C\n\t"
        "	cmp r1, r0\n\t"
        "	ble _081D7A96\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	str r5, [sp]\n\t"
        "	movs r1, #6\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0x10\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	b _081D7A96\n\t"
        "	.align 2, 0\n\t"
        "_081D7A5C: .4byte SPECIAL_Script_ClearHeldMovement\n\t"
        "_081D7A60:\n\t"
        "	ldrh r0, [r4, #4]\n\t"
        "	adds r0, #0x10\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	movs r1, #0xf0\n\t"
        "	lsls r1, r1, #7\n\t"
        "	movs r2, #0xc0\n\t"
        "	lsls r2, r2, #5\n\t"
        "	ldrh r0, [r4, #8]\n\t"
        "	adds r0, #0x20\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	str r0, [sp]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #4]\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	str r0, [sp, #8]\n\t"
        "	movs r0, #0\n\t"
        "	str r0, [sp, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	movs r3, #0x78\n\t"
        "	bl SetBgAffine\n\t"
        "	adds r0, r6, #0\n\t"
        "	bl Task_RayTakesFlightEnd\n\t"
        "_081D7A96:\n\t"
        "	add sp, #0x10\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayTakesFlightEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r4, r0, #0x18\n\t"
        "	ldr r0, _081D7AD4\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D7ACE\n\t"
        "	movs r0, #0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	ldr r0, _081D7AD8\n\t"
        "	lsls r1, r4, #2\n\t"
        "	adds r1, r1, r4\n\t"
        "	lsls r1, r1, #3\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r0, _081D7ADC\n\t"
        "	str r0, [r1]\n\t"
        "_081D7ACE:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7AD4: .4byte gPaletteFade\n\t"
        "_081D7AD8: .4byte gTasks\n\t"
        "_081D7ADC: .4byte Task_SetNextAnim + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D7AE0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	lsls r0, r5, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D7B7C\n\t"
        "	adds r4, r0, r1\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r0, #3\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D7B90\n\t"
        "	ldr r0, _081D7B80\n\t"
        "	ldr r3, _081D7B84\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r2, [r4, r1]\n\t"
        "	lsls r2, r2, #1\n\t"
        "	adds r1, r2, r3\n\t"
        "	ldrb r1, [r1]\n\t"
        "	lsls r1, r1, #0x18\n\t"
        "	asrs r1, r1, #0x18\n\t"
        "	lsls r1, r1, #0x12\n\t"
        "	movs r6, #0xf0\n\t"
        "	lsls r6, r6, #0xf\n\t"
        "	adds r1, r1, r6\n\t"
        "	asrs r1, r1, #0x10\n\t"
        "	adds r3, #1\n\t"
        "	adds r2, r2, r3\n\t"
        "	ldrb r2, [r2]\n\t"
        "	lsls r2, r2, #0x18\n\t"
        "	asrs r2, r2, #0x18\n\t"
        "	lsls r2, r2, #0x12\n\t"
        "	movs r3, #0xa0\n\t"
        "	lsls r3, r3, #0xf\n\t"
        "	adds r2, r2, r3\n\t"
        "	asrs r2, r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	ldr r1, _081D7B88\n\t"
        "	lsls r3, r0, #4\n\t"
        "	adds r3, r3, r0\n\t"
        "	lsls r3, r3, #2\n\t"
        "	adds r3, r3, r1\n\t"
        "	movs r0, #0\n\t"
        "	ldrsb r0, [r4, r0]\n\t"
        "	strh r0, [r3, #0x2e]\n\t"
        "	ldrb r2, [r3, #1]\n\t"
        "	movs r1, #0xd\n\t"
        "	rsbs r1, r1, #0\n\t"
        "	adds r0, r1, #0\n\t"
        "	ands r0, r2\n\t"
        "	movs r2, #4\n\t"
        "	orrs r0, r2\n\t"
        "	movs r2, #3\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r3, #1]\n\t"
        "	ldrb r0, [r3, #5]\n\t"
        "	ands r1, r0\n\t"
        "	movs r0, #8\n\t"
        "	orrs r1, r0\n\t"
        "	strb r1, [r3, #5]\n\t"
        "	adds r0, r3, #0\n\t"
        "	bl InitSpriteAffineAnim\n\t"
        "	ldrh r1, [r4]\n\t"
        "	movs r6, #0\n\t"
        "	ldrsh r0, [r4, r6]\n\t"
        "	cmp r0, #9\n\t"
        "	bne _081D7B8C\n\t"
        "	adds r0, r5, #0\n\t"
        "	bl DestroyTask\n\t"
        "	b _081D7B96\n\t"
        "	.align 2, 0\n\t"
        "_081D7B7C: .4byte gUnknown_3005B68\n\t"
        "_081D7B80: .4byte sSpriteTemplate_TakesFlight_Smoke\n\t"
        "_081D7B84: .4byte sTakesFlight_SmokeCoords\n\t"
        "_081D7B88: .4byte gSprites\n\t"
        "_081D7B8C:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4]\n\t"
        "_081D7B90:\n\t"
        "	ldrh r0, [r4, #2]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "_081D7B96:\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void SpriteCB_TakesFlight_Smoke(struct Sprite *sprite)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	adds r2, r0, #0\n\t"
        "	movs r1, #0x30\n\t"
        "	ldrsh r0, [r2, r1]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D7BAC\n\t"
        "	strh r0, [r2, #0x24]\n\t"
        "	b _081D7BD6\n\t"
        "_081D7BAC:\n\t"
        "	ldr r1, _081D7BE8\n\t"
        "	movs r3, #0x2e\n\t"
        "	ldrsh r0, [r2, r3]\n\t"
        "	lsls r0, r0, #1\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldrb r0, [r0]\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	asrs r0, r0, #0x18\n\t"
        "	ldrh r3, [r2, #0x24]\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r0, [r2, #0x24]\n\t"
        "	movs r3, #0x2e\n\t"
        "	ldrsh r0, [r2, r3]\n\t"
        "	lsls r0, r0, #1\n\t"
        "	adds r1, #1\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldrb r0, [r0]\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	asrs r0, r0, #0x18\n\t"
        "	ldrh r1, [r2, #0x26]\n\t"
        "	adds r0, r0, r1\n\t"
        "_081D7BD6:\n\t"
        "	strh r0, [r2, #0x26]\n\t"
        "	ldrh r0, [r2, #0x30]\n\t"
        "	adds r0, #1\n\t"
        "	movs r1, #0xf\n\t"
        "	ands r0, r1\n\t"
        "	strh r0, [r2, #0x30]\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7BE8: .4byte sTakesFlight_SmokeCoords\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D7BEC(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	bl ResetVramOamAndBgCntRegs\n\t"
        "	movs r0, #0\n\t"
        "	bl ResetBgsAndClearDma3BusyFlags\n\t"
        "	ldr r1, _081D7C80\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #4\n\t"
        "	bl InitBgsFromTemplates\n\t"
        "	ldr r4, _081D7C84\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D7C88\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #1\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D7C8C\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #2\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D7C90\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #3\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	bl ResetAllBgsCoordinates\n\t"
        "	movs r0, #0\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #1\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #2\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #3\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r1, #0x82\n\t"
        "	lsls r1, r1, #5\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #1\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #2\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #3\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #0x50\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7C80: .4byte sBgTemplates_Descends\n\t"
        "_081D7C84: .4byte gUnknown_203CC2C\n\t"
        "_081D7C88: .4byte 0x00000804\n\t"
        "_081D7C8C: .4byte 0x00001004\n\t"
        "_081D7C90: .4byte 0x00001804\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D7C94(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	sub sp, #0xc\n\t"
        "	bl ResetTempTileDataBuffers\n\t"
        "	ldr r1, _081D7D40\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D7D44\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	add r7, sp, #8\n\t"
        "_081D7CBC:\n\t"
        "	bl FreeTempTileDataBuffersIfPossible\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r6, r0, #0x18\n\t"
        "	cmp r6, #0\n\t"
        "	bne _081D7CBC\n\t"
        "	ldr r0, _081D7D48\n\t"
        "	ldr r5, _081D7D4C\n\t"
        "	ldr r1, [r5]\n\t"
        "	adds r1, #4\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D7D50\n\t"
        "	ldr r1, [r5]\n\t"
        "	ldr r4, _081D7D54\n\t"
        "	adds r1, r1, r4\n\t"
        "	bl LZDecompressVram\n\t"
        "	str r6, [sp, #4]\n\t"
        "	ldr r1, [r5]\n\t"
        "	ldr r0, _081D7D58\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r2, _081D7D5C\n\t"
        "	add r0, sp, #4\n\t"
        "	bl CpuFastSet\n\t"
        "	ldr r1, [r5]\n\t"
        "	adds r4, r1, r4\n\t"
        "	ldr r0, _081D7D60\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r2, #0x80\n\t"
        "	lsls r2, r2, #2\n\t"
        "	adds r0, r4, #0\n\t"
        "	bl CpuFastSet\n\t"
        "	str r6, [sp, #8]\n\t"
        "	ldr r1, [r5]\n\t"
        "	ldr r0, _081D7D64\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r2, _081D7D68\n\t"
        "	adds r0, r7, #0\n\t"
        "	bl CpuFastSet\n\t"
        "	ldr r0, _081D7D6C\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x40\n\t"
        "	bl LoadCompressedPalette\n\t"
        "	ldr r0, _081D7D70\n\t"
        "	ldr r1, _081D7D74\n\t"
        "	strh r1, [r0]\n\t"
        "	ldr r0, _081D7D78\n\t"
        "	strh r1, [r0]\n\t"
        "	ldr r0, _081D7D7C\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D7D80\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D7D84\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	add sp, #0xc\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7D40: .4byte gRaySceneDescends_Light_Gfx\n\t"
        "_081D7D44: .4byte gRaySceneDescends_Bg_Gfx\n\t"
        "_081D7D48: .4byte gRaySceneDescends_Light_Tilemap\n\t"
        "_081D7D4C: .4byte gUnknown_203CC2C\n\t"
        "_081D7D50: .4byte gRaySceneDescends_Bg_Tilemap\n\t"
        "_081D7D54: .4byte 0x00001804\n\t"
        "_081D7D58: .4byte 0x00001004\n\t"
        "_081D7D5C: .4byte 0x01000200\n\t"
        "_081D7D60: .4byte 0x00000804\n\t"
        "_081D7D64: .4byte 0x00000904\n\t"
        "_081D7D68: .4byte 0x010000D0\n\t"
        "_081D7D6C: .4byte gRaySceneDescends_Bg_Pal\n\t"
        "_081D7D70: .4byte gPlttBufferUnfaded\n\t"
        "_081D7D74: .4byte 0x00007FFF\n\t"
        "_081D7D78: .4byte gPlttBufferFaded\n\t"
        "_081D7D7C: .4byte sSpriteSheet_Descends_Rayquaza\n\t"
        "_081D7D80: .4byte sSpriteSheet_Descends_RayquazaTail\n\t"
        "_081D7D84: .4byte sSpritePal_Descends_Rayquaza\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D7D88(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	movs r0, #6\n\t"
        "	bl GetGpuReg\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	lsrs r2, r0, #0x10\n\t"
        "	adds r1, r2, #0\n\t"
        "	subs r1, #0x18\n\t"
        "	lsls r0, r1, #0x10\n\t"
        "	lsrs r0, r0, #0x10\n\t"
        "	cmp r0, #0x6f\n\t"
        "	bhi _081D7DC8\n\t"
        "	ldr r0, _081D7DB8\n\t"
        "	ldr r0, [r0]\n\t"
        "	ldr r3, _081D7DBC\n\t"
        "	adds r0, r0, r3\n\t"
        "	movs r3, #0\n\t"
        "	ldrsh r0, [r0, r3]\n\t"
        "	cmp r1, r0\n\t"
        "	bgt _081D7DC8\n\t"
        "	ldr r1, _081D7DC0\n\t"
        "	ldr r3, _081D7DC4\n\t"
        "	b _081D7DCE\n\t"
        "	.align 2, 0\n\t"
        "_081D7DB8: .4byte gUnknown_203CC2C\n\t"
        "_081D7DBC: .4byte 0x00002008\n\t"
        "_081D7DC0: .4byte 0x04000052\n\t"
        "_081D7DC4: .4byte 0x00000D08\n\t"
        "_081D7DC8:\n\t"
        "	ldr r1, _081D7DF4\n\t"
        "	movs r3, #0x80\n\t"
        "	lsls r3, r3, #5\n\t"
        "_081D7DCE:\n\t"
        "	adds r0, r3, #0\n\t"
        "	strh r0, [r1]\n\t"
        "	cmp r2, #0\n\t"
        "	bne _081D7E1C\n\t"
        "	ldr r0, _081D7DF8\n\t"
        "	ldr r1, [r0]\n\t"
        "	ldr r2, _081D7DFC\n\t"
        "	adds r3, r1, r2\n\t"
        "	ldrh r4, [r3]\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r2, [r3, r1]\n\t"
        "	ldr r1, _081D7E00\n\t"
        "	adds r5, r0, #0\n\t"
        "	cmp r2, r1\n\t"
        "	bgt _081D7E10\n\t"
        "	cmp r2, #0x27\n\t"
        "	bgt _081D7E04\n\t"
        "	adds r0, r4, #4\n\t"
        "	b _081D7E0E\n\t"
        "	.align 2, 0\n\t"
        "_081D7DF4: .4byte 0x04000052\n\t"
        "_081D7DF8: .4byte gUnknown_203CC2C\n\t"
        "_081D7DFC: .4byte 0x00002008\n\t"
        "_081D7E00: .4byte 0x00001FFF\n\t"
        "_081D7E04:\n\t"
        "	cmp r2, #0x4f\n\t"
        "	bgt _081D7E0C\n\t"
        "	adds r0, r4, #2\n\t"
        "	b _081D7E0E\n\t"
        "_081D7E0C:\n\t"
        "	adds r0, r4, #1\n\t"
        "_081D7E0E:\n\t"
        "	strh r0, [r3]\n\t"
        "_081D7E10:\n\t"
        "	ldr r1, [r5]\n\t"
        "	ldr r2, _081D7E24\n\t"
        "	adds r1, r1, r2\n\t"
        "	ldrh r0, [r1]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r1]\n\t"
        "_081D7E1C:\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7E24: .4byte 0x0000200A\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayDescendsAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r4, r0, #2\n\t"
        "	adds r4, r4, r0\n\t"
        "	lsls r4, r4, #3\n\t"
        "	ldr r6, _081D7E9C\n\t"
        "	adds r5, r4, r6\n\t"
        "	bl sub_081D7BEC\n\t"
        "	bl sub_081D7C94\n\t"
        "	ldr r1, _081D7EA0\n\t"
        "	movs r0, #0x50\n\t"
        "	bl SetGpuRegBits\n\t"
        "	movs r0, #0x80\n\t"
        "	lsls r0, r0, #5\n\t"
        "	mov r8, r0\n\t"
        "	movs r0, #0x52\n\t"
        "	mov r1, r8\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0\n\t"
        "	bl BlendPalettes\n\t"
        "	ldr r0, _081D7EA4\n\t"
        "	bl SetVBlankCallback\n\t"
        "	ldr r0, _081D7EA8\n\t"
        "	ldr r0, [r0]\n\t"
        "	ldr r1, _081D7EAC\n\t"
        "	adds r2, r0, r1\n\t"
        "	movs r1, #0\n\t"
        "	strh r1, [r2]\n\t"
        "	ldr r2, _081D7EB0\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r1, [r0]\n\t"
        "	strh r1, [r5]\n\t"
        "	strh r1, [r5, #2]\n\t"
        "	strh r1, [r5, #4]\n\t"
        "	strh r1, [r5, #6]\n\t"
        "	mov r0, r8\n\t"
        "	strh r0, [r5, #8]\n\t"
        "	subs r6, #8\n\t"
        "	adds r4, r4, r6\n\t"
        "	ldr r0, _081D7EB4\n\t"
        "	str r0, [r4]\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7E9C: .4byte gUnknown_3005B68\n\t"
        "_081D7EA0: .4byte 0x00001E41\n\t"
        "_081D7EA4: .4byte VBlankCB_RayquazaScene + 1\n\t"
        "_081D7EA8: .4byte gUnknown_203CC2C\n\t"
        "_081D7EAC: .4byte 0x00002008\n\t"
        "_081D7EB0: .4byte 0x0000200A\n\t"
        "_081D7EB4: .4byte Task_HandleRayDescends + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleRayDescends(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	sub sp, #4\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r4, r0, #0x18\n\t"
        "	lsls r0, r4, #2\n\t"
        "	adds r0, r0, r4\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D7EDC\n\t"
        "	adds r5, r0, r1\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r5, r1]\n\t"
        "	cmp r0, #4\n\t"
        "	bhi _081D7FB0\n\t"
        "	lsls r0, r0, #2\n\t"
        "	ldr r1, _081D7EE0\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r0, [r0]\n\t"
        "	mov pc, r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7EDC: .4byte gUnknown_3005B68\n\t"
        "_081D7EE0: .4byte _081D7EE4\n\t"
        "_081D7EE4:\n\t"
        "	.4byte _081D7EF8\n\t"
        "	.4byte _081D7F18\n\t"
        "	.4byte _081D7F56\n\t"
        "	.4byte _081D7F76\n\t"
        "	.4byte _081D7F92\n\t"
        "_081D7EF8:\n\t"
        "	ldrh r1, [r5, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r5, r2]\n\t"
        "	cmp r0, #8\n\t"
        "	bne _081D7F70\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	strh r4, [r5, #2]\n\t"
        "	b _081D7F8A\n\t"
        "_081D7F18:\n\t"
        "	ldr r0, _081D7F48\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r1, r0, #0x18\n\t"
        "	cmp r1, #0\n\t"
        "	bne _081D7FB0\n\t"
        "	ldrh r2, [r5, #2]\n\t"
        "	movs r3, #2\n\t"
        "	ldrsh r0, [r5, r3]\n\t"
        "	cmp r0, #0xa\n\t"
        "	bne _081D7F50\n\t"
        "	strh r1, [r5, #2]\n\t"
        "	ldrh r0, [r5]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5]\n\t"
        "	ldr r0, _081D7F4C\n\t"
        "	bl SetHBlankCallback\n\t"
        "	movs r0, #3\n\t"
        "	bl EnableInterrupts\n\t"
        "	b _081D7FB0\n\t"
        "	.align 2, 0\n\t"
        "_081D7F48: .4byte gPaletteFade\n\t"
        "_081D7F4C: .4byte sub_081D7D88 + 1\n\t"
        "_081D7F50:\n\t"
        "	adds r0, r2, #1\n\t"
        "	strh r0, [r5, #2]\n\t"
        "	b _081D7FB0\n\t"
        "_081D7F56:\n\t"
        "	ldrh r1, [r5, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r5, r2]\n\t"
        "	cmp r0, #0x50\n\t"
        "	bne _081D7F70\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r5, #2]\n\t"
        "	ldrh r0, [r5]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5]\n\t"
        "	bl sub_081D8008\n\t"
        "	b _081D7FB0\n\t"
        "_081D7F70:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r5, #2]\n\t"
        "	b _081D7FB0\n\t"
        "_081D7F76:\n\t"
        "	ldrh r0, [r5, #2]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5, #2]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	movs r1, #0xb8\n\t"
        "	lsls r1, r1, #0x11\n\t"
        "	cmp r0, r1\n\t"
        "	bne _081D7FB0\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r5, #2]\n\t"
        "_081D7F8A:\n\t"
        "	ldrh r0, [r5]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5]\n\t"
        "	b _081D7FB0\n\t"
        "_081D7F92:\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0\n\t"
        "	str r1, [sp]\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0x10\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	ldr r1, _081D7FB8\n\t"
        "	lsls r0, r4, #2\n\t"
        "	adds r0, r0, r4\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r1, _081D7FBC\n\t"
        "	str r1, [r0]\n\t"
        "_081D7FB0:\n\t"
        "	add sp, #4\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7FB8: .4byte gTasks\n\t"
        "_081D7FBC: .4byte Task_RayDescendsEnd + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayDescendsEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r4, r0, #0x18\n\t"
        "	ldr r0, _081D7FFC\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D7FF4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	movs r0, #0\n\t"
        "	bl SetHBlankCallback\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	ldr r0, _081D8000\n\t"
        "	lsls r1, r4, #2\n\t"
        "	adds r1, r1, r4\n\t"
        "	lsls r1, r1, #3\n\t"
        "	adds r1, r1, r0\n\t"
        "	ldr r0, _081D8004\n\t"
        "	str r0, [r1]\n\t"
        "_081D7FF4:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D7FFC: .4byte gPaletteFade\n\t"
        "_081D8000: .4byte gTasks\n\t"
        "_081D8004: .4byte Task_SetNextAnim + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8008(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	ldr r0, _081D8078\n\t"
        "	movs r1, #0xa0\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	adds r6, r0, #0\n\t"
        "	lsls r6, r6, #0x18\n\t"
        "	lsrs r6, r6, #0x18\n\t"
        "	lsls r5, r6, #4\n\t"
        "	adds r5, r5, r6\n\t"
        "	lsls r5, r5, #2\n\t"
        "	ldr r4, _081D807C\n\t"
        "	adds r0, r5, r4\n\t"
        "	mov r8, r0\n\t"
        "	ldr r0, _081D8080\n\t"
        "	movs r2, #0x30\n\t"
        "	rsbs r2, r2, #0\n\t"
        "	movs r1, #0xb8\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	mov r1, r8\n\t"
        "	strh r0, [r1]\n\t"
        "	adds r3, r4, #0\n\t"
        "	subs r3, #0x2e\n\t"
        "	subs r4, #0x12\n\t"
        "	adds r4, r5, r4\n\t"
        "	ldr r0, _081D8084\n\t"
        "	str r0, [r4]\n\t"
        "	adds r5, r5, r3\n\t"
        "	ldrb r0, [r5, #5]\n\t"
        "	movs r2, #0xc\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r5, #5]\n\t"
        "	movs r4, #0\n\t"
        "	ldrsh r1, [r1, r4]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrb r1, [r0, #5]\n\t"
        "	orrs r1, r2\n\t"
        "	strb r1, [r0, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r1}\n\t"
        "	bx r1\n\t"
        "	.align 2, 0\n\t"
        "_081D8078: .4byte sSpriteTemplate_Descends_Rayquaza\n\t"
        "_081D807C: .4byte gUnknown_20205DA\n\t"
        "_081D8080: .4byte sSpriteTemplate_Descends_RayquazaTail\n\t"
        "_081D8084: .4byte sub_081D8088 + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8088(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	adds r5, r0, #0\n\t"
        "	adds r4, r5, #0\n\t"
        "	adds r4, #0x2e\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	cmp r1, #0\n\t"
        "	bne _081D80A0\n\t"
        "	movs r0, #0xc\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #8\n\t"
        "	b _081D80FE\n\t"
        "_081D80A0:\n\t"
        "	movs r0, #0x80\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D80B0\n\t"
        "	movs r0, #9\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #7\n\t"
        "	b _081D80FE\n\t"
        "_081D80B0:\n\t"
        "	movs r0, #0x86\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D80C0\n\t"
        "	movs r0, #8\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #6\n\t"
        "	b _081D80FE\n\t"
        "_081D80C0:\n\t"
        "	movs r0, #0x8c\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D80D0\n\t"
        "	movs r0, #7\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #5\n\t"
        "	b _081D80FE\n\t"
        "_081D80D0:\n\t"
        "	movs r0, #0x92\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D80E0\n\t"
        "	movs r0, #6\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #4\n\t"
        "	b _081D80FE\n\t"
        "_081D80E0:\n\t"
        "	movs r0, #0x98\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D80F0\n\t"
        "	movs r0, #5\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #3\n\t"
        "	b _081D80FE\n\t"
        "_081D80F0:\n\t"
        "	movs r0, #0xa0\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D8100\n\t"
        "	movs r0, #4\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #2\n\t"
        "_081D80FE:\n\t"
        "	strh r0, [r4, #8]\n\t"
        "_081D8100:\n\t"
        "	movs r1, #4\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	movs r2, #6\n\t"
        "	ldrsh r1, [r4, r2]\n\t"
        "	bl __modsi3\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D812A\n\t"
        "	ldrh r0, [r5, #0x24]\n\t"
        "	subs r0, #1\n\t"
        "	strh r0, [r5, #0x24]\n\t"
        "	ldr r2, _081D8160\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x24]\n\t"
        "	subs r1, #1\n\t"
        "	strh r1, [r0, #0x24]\n\t"
        "_081D812A:\n\t"
        "	movs r1, #4\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	movs r2, #8\n\t"
        "	ldrsh r1, [r4, r2]\n\t"
        "	bl __modsi3\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8154\n\t"
        "	ldrh r0, [r5, #0x26]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5, #0x26]\n\t"
        "	ldr r2, _081D8160\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x26]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x26]\n\t"
        "_081D8154:\n\t"
        "	ldrh r0, [r4, #4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8160: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8164(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	bl ResetVramOamAndBgCntRegs\n\t"
        "	movs r0, #0\n\t"
        "	bl ResetBgsAndClearDma3BusyFlags\n\t"
        "	ldr r1, _081D81F8\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #4\n\t"
        "	bl InitBgsFromTemplates\n\t"
        "	ldr r4, _081D81FC\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D8200\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #1\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D8204\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #2\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D8208\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #3\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	bl ResetAllBgsCoordinates\n\t"
        "	movs r0, #0\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #1\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #2\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #3\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r1, #0xc1\n\t"
        "	lsls r1, r1, #6\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #1\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #2\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #3\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #0x50\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D81F8: .4byte sBgTemplates_Charges\n\t"
        "_081D81FC: .4byte gUnknown_203CC2C\n\t"
        "_081D8200: .4byte 0x00000804\n\t"
        "_081D8204: .4byte 0x00001004\n\t"
        "_081D8208: .4byte 0x00001804\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D820C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #4\n\t"
        "	bl ResetTempTileDataBuffers\n\t"
        "	ldr r1, _081D828C\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D8290\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #2\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D8294\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #3\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "_081D8240:\n\t"
        "	bl FreeTempTileDataBuffersIfPossible\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8240\n\t"
        "	ldr r0, _081D8298\n\t"
        "	ldr r4, _081D829C\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D82A0\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D82A4\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D82A8\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D82AC\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D82B0\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D82B4\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D82B8\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x80\n\t"
        "	bl LoadCompressedPalette\n\t"
        "	add sp, #4\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D828C: .4byte gRaySceneCharges_Rayquaza_Gfx\n\t"
        "_081D8290: .4byte gRaySceneCharges_Streaks_Gfx\n\t"
        "_081D8294: .4byte gRaySceneCharges_Bg_Gfx\n\t"
        "_081D8298: .4byte gRaySceneCharges_Orbs_Tilemap\n\t"
        "_081D829C: .4byte gUnknown_203CC2C\n\t"
        "_081D82A0: .4byte gRaySceneCharges_Rayquaza_Tilemap\n\t"
        "_081D82A4: .4byte 0x00000804\n\t"
        "_081D82A8: .4byte gRaySceneCharges_Streaks_Tilemap\n\t"
        "_081D82AC: .4byte 0x00001004\n\t"
        "_081D82B0: .4byte gRaySceneCharges_Bg_Tilemap\n\t"
        "_081D82B4: .4byte 0x00001804\n\t"
        "_081D82B8: .4byte gRaySceneCharges_Bg_Pal\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayChargesAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r4, r0, #2\n\t"
        "	adds r4, r4, r0\n\t"
        "	lsls r4, r4, #3\n\t"
        "	ldr r5, _081D830C\n\t"
        "	adds r6, r4, r5\n\t"
        "	bl sub_081D8164\n\t"
        "	bl sub_081D820C\n\t"
        "	bl sub_081D6204\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0\n\t"
        "	bl BlendPalettes\n\t"
        "	ldr r0, _081D8310\n\t"
        "	bl SetVBlankCallback\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r6]\n\t"
        "	strh r0, [r6, #2]\n\t"
        "	ldr r0, _081D8314\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r6, #4]\n\t"
        "	subs r5, #8\n\t"
        "	adds r4, r4, r5\n\t"
        "	ldr r0, _081D8318\n\t"
        "	str r0, [r4]\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D830C: .4byte gUnknown_3005B68\n\t"
        "_081D8310: .4byte VBlankCB_RayquazaScene + 1\n\t"
        "_081D8314: .4byte sub_081D8414 + 1\n\t"
        "_081D8318: .4byte Task_HandleRayCharges + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleRayCharges(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	sub sp, #4\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r6, r0, #0x18\n\t"
        "	lsls r0, r6, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D836C\n\t"
        "	adds r4, r0, r1\n\t"
        "	bl sub_081D84F0\n\t"
        "	ldrh r1, [r4, #6]\n\t"
        "	movs r0, #7\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8352\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #1\n\t"
        "	bgt _081D8352\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x59\n\t"
        "	bgt _081D8352\n\t"
        "	movs r0, #0x67\n\t"
        "	bl PlaySE\n\t"
        "_081D8352:\n\t"
        "	ldrh r0, [r4, #6]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r5, [r4, r0]\n\t"
        "	cmp r5, #1\n\t"
        "	beq _081D8398\n\t"
        "	cmp r5, #1\n\t"
        "	bgt _081D8370\n\t"
        "	cmp r5, #0\n\t"
        "	beq _081D837A\n\t"
        "	b _081D8402\n\t"
        "	.align 2, 0\n\t"
        "_081D836C: .4byte gUnknown_3005B68\n\t"
        "_081D8370:\n\t"
        "	cmp r5, #2\n\t"
        "	beq _081D83C8\n\t"
        "	cmp r5, #3\n\t"
        "	beq _081D83E4\n\t"
        "	b _081D8402\n\t"
        "_081D837A:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #8\n\t"
        "	bne _081D83DE\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	str r5, [sp]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	strh r5, [r4, #2]\n\t"
        "	b _081D83D6\n\t"
        "_081D8398:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x7f\n\t"
        "	bne _081D83DE\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	ldr r2, _081D83C0\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D83C4\n\t"
        "	b _081D8400\n\t"
        "	.align 2, 0\n\t"
        "_081D83C0: .4byte gTasks\n\t"
        "_081D83C4: .4byte sub_081D8468 + 1\n\t"
        "_081D83C8:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0xc\n\t"
        "	bne _081D83DE\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r4, #2]\n\t"
        "_081D83D6:\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	b _081D8402\n\t"
        "_081D83DE:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	b _081D8402\n\t"
        "_081D83E4:\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0\n\t"
        "	str r1, [sp]\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0x10\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	ldr r1, _081D840C\n\t"
        "	lsls r0, r6, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r1, _081D8410\n\t"
        "_081D8400:\n\t"
        "	str r1, [r0]\n\t"
        "_081D8402:\n\t"
        "	add sp, #4\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D840C: .4byte gTasks\n\t"
        "_081D8410: .4byte Task_RayChargesEnd + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8414(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D8464\n\t"
        "	adds r5, r1, r0\n\t"
        "	ldrh r1, [r5, #0x1e]\n\t"
        "	movs r0, #3\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8458\n\t"
        "	bl Random\n\t"
        "	movs r4, #7\n\t"
        "	adds r1, r4, #0\n\t"
        "	ands r1, r0\n\t"
        "	subs r1, #4\n\t"
        "	lsls r1, r1, #8\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #0\n\t"
        "	bl ChangeBgX\n\t"
        "	bl Random\n\t"
        "	ands r4, r0\n\t"
        "	subs r4, #4\n\t"
        "	lsls r4, r4, #8\n\t"
        "	movs r0, #1\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #0\n\t"
        "	bl ChangeBgY\n\t"
        "_081D8458:\n\t"
        "	ldrh r0, [r5, #0x1e]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r5, #0x1e]\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8464: .4byte gUnknown_3005B68\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8468(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D84A8\n\t"
        "	adds r4, r1, r0\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D84AC\n\t"
        "	movs r0, #1\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0\n\t"
        "	bl ChangeBgX\n\t"
        "	movs r0, #1\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0\n\t"
        "	bl ChangeBgY\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	movs r0, #0xa\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	movs r1, #1\n\t"
        "	rsbs r1, r1, #0\n\t"
        "	adds r0, r1, #0\n\t"
        "	b _081D84E6\n\t"
        "	.align 2, 0\n\t"
        "_081D84A8: .4byte gUnknown_3005B68\n\t"
        "_081D84AC:\n\t"
        "	cmp r0, #1\n\t"
        "	bne _081D84E8\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r1, r1, #8\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #2\n\t"
        "	bl ChangeBgX\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r1, r1, #8\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #1\n\t"
        "	bl ChangeBgY\n\t"
        "	ldrh r0, [r4, #4]\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	adds r0, r0, r1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	movs r1, #0xa\n\t"
        "	rsbs r1, r1, #0\n\t"
        "	cmp r0, r1\n\t"
        "	bne _081D84E8\n\t"
        "	movs r1, #4\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	rsbs r0, r0, #0\n\t"
        "_081D84E6:\n\t"
        "	strh r0, [r4, #4]\n\t"
        "_081D84E8:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D84F0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	movs r4, #0x80\n\t"
        "	lsls r4, r4, #3\n\t"
        "	movs r0, #2\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #2\n\t"
        "	bl ChangeBgX\n\t"
        "	movs r0, #2\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #1\n\t"
        "	bl ChangeBgY\n\t"
        "	movs r4, #0x80\n\t"
        "	lsls r4, r4, #4\n\t"
        "	movs r0, #0\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #2\n\t"
        "	bl ChangeBgX\n\t"
        "	movs r0, #0\n\t"
        "	adds r1, r4, #0\n\t"
        "	movs r2, #1\n\t"
        "	bl ChangeBgY\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayChargesEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r5, r1, #3\n\t"
        "	ldr r6, _081D8568\n\t"
        "	adds r4, r5, r6\n\t"
        "	bl sub_081D84F0\n\t"
        "	ldr r0, _081D856C\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8562\n\t"
        "	movs r0, #0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	bl sub_081D6240\n\t"
        "	ldrb r0, [r4, #4]\n\t"
        "	bl DestroyTask\n\t"
        "	adds r0, r6, #0\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r5, r0\n\t"
        "	ldr r1, _081D8570\n\t"
        "	str r1, [r0]\n\t"
        "_081D8562:\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8568: .4byte gUnknown_3005B68\n\t"
        "_081D856C: .4byte gPaletteFade\n\t"
        "_081D8570: .4byte Task_SetNextAnim + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8574(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	bl ResetVramOamAndBgCntRegs\n\t"
        "	movs r0, #0\n\t"
        "	bl ResetBgsAndClearDma3BusyFlags\n\t"
        "	ldr r1, _081D85F0\n\t"
        "	movs r0, #1\n\t"
        "	movs r2, #3\n\t"
        "	bl InitBgsFromTemplates\n\t"
        "	ldr r4, _081D85F4\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D85F8\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #1\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r0, _081D85FC\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #2\n\t"
        "	bl SetBgTilemapBuffer\n\t"
        "	bl ResetAllBgsCoordinates\n\t"
        "	movs r0, #0\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #1\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r0, #2\n\t"
        "	bl ScheduleBgCopyTilemapToVram\n\t"
        "	movs r1, #0xc1\n\t"
        "	lsls r1, r1, #6\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #0\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #1\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #2\n\t"
        "	bl ShowBg\n\t"
        "	movs r0, #0x50\n\t"
        "	movs r1, #0\n\t"
        "	bl SetGpuReg\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D85F0: .4byte sBgTemplates_ChasesAway\n\t"
        "_081D85F4: .4byte gUnknown_203CC2C\n\t"
        "_081D85F8: .4byte 0x00000804\n\t"
        "_081D85FC: .4byte 0x00001004\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8600(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	sub sp, #4\n\t"
        "	bl ResetTempTileDataBuffers\n\t"
        "	ldr r1, _081D86A4\n\t"
        "	movs r4, #0\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #2\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "	ldr r1, _081D86A8\n\t"
        "	str r4, [sp]\n\t"
        "	movs r0, #0\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0\n\t"
        "	bl DecompressAndCopyTileDataToVram\n\t"
        "_081D8626:\n\t"
        "	bl FreeTempTileDataBuffersIfPossible\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8626\n\t"
        "	ldr r0, _081D86AC\n\t"
        "	ldr r4, _081D86B0\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D86B4\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D86B8\n\t"
        "	ldr r1, [r4]\n\t"
        "	adds r1, #4\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D86BC\n\t"
        "	ldr r1, [r4]\n\t"
        "	ldr r2, _081D86C0\n\t"
        "	adds r1, r1, r2\n\t"
        "	bl LZDecompressVram\n\t"
        "	ldr r0, _081D86C4\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x60\n\t"
        "	bl LoadCompressedPalette\n\t"
        "	ldr r0, _081D86C8\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86CC\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86D0\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86D4\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86D8\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86DC\n\t"
        "	bl LoadCompressedSpriteSheet\n\t"
        "	ldr r0, _081D86E0\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	ldr r0, _081D86E4\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	ldr r0, _081D86E8\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	ldr r0, _081D86EC\n\t"
        "	bl LoadCompressedSpritePalette\n\t"
        "	add sp, #4\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D86A4: .4byte gRaySceneChasesAway_Ring_Gfx\n\t"
        "_081D86A8: .4byte gRaySceneChasesAway_Light_Gfx\n\t"
        "_081D86AC: .4byte gRaySceneChasesAway_Bg_Tilemap\n\t"
        "_081D86B0: .4byte gUnknown_203CC2C\n\t"
        "_081D86B4: .4byte 0x00000804\n\t"
        "_081D86B8: .4byte gRaySceneChasesAway_Light_Tilemap\n\t"
        "_081D86BC: .4byte gRaySceneChasesAway_Ring_Tilemap\n\t"
        "_081D86C0: .4byte 0x00001004\n\t"
        "_081D86C4: .4byte gRaySceneChasesAway_Bg_Pal\n\t"
        "_081D86C8: .4byte sSpriteSheet_ChasesAway_Groudon\n\t"
        "_081D86CC: .4byte sSpriteSheet_ChasesAway_GroudonTail\n\t"
        "_081D86D0: .4byte sSpriteSheet_ChasesAway_Kyogre\n\t"
        "_081D86D4: .4byte sSpriteSheet_ChasesAway_Rayquaza\n\t"
        "_081D86D8: .4byte sSpriteSheet_ChasesAway_RayquazaTail\n\t"
        "_081D86DC: .4byte sSpriteSheet_ChasesAway_KyogreSplash\n\t"
        "_081D86E0: .4byte sSpritePal_ChasesAway_Groudon\n\t"
        "_081D86E4: .4byte sSpritePal_ChasesAway_Kyogre\n\t"
        "_081D86E8: .4byte sSpritePal_ChasesAway_Rayquaza\n\t"
        "_081D86EC: .4byte sSpritePal_ChasesAway_KyogreSplash\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayChasesAwayAnim(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r4, r0, #2\n\t"
        "	adds r4, r4, r0\n\t"
        "	lsls r4, r4, #3\n\t"
        "	ldr r5, _081D87A4\n\t"
        "	adds r6, r4, r5\n\t"
        "	bl sub_081D8574\n\t"
        "	bl sub_081D8600\n\t"
        "	bl sub_081D6204\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #3\n\t"
        "	movs r0, #0\n\t"
        "	bl ClearGpuRegBits\n\t"
        "	ldr r1, _081D87A8\n\t"
        "	movs r0, #0x50\n\t"
        "	bl SetGpuReg\n\t"
        "	ldr r1, _081D87AC\n\t"
        "	movs r0, #0x52\n\t"
        "	bl SetGpuReg\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0\n\t"
        "	bl BlendPalettes\n\t"
        "	ldr r0, _081D87B0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r6]\n\t"
        "	strh r0, [r6, #2]\n\t"
        "	subs r5, #8\n\t"
        "	adds r4, r4, r5\n\t"
        "	ldr r0, _081D87B4\n\t"
        "	str r0, [r4]\n\t"
        "	ldr r0, _081D87B8\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r6, #4]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r6, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r5\n\t"
        "	movs r2, #0\n\t"
        "	strh r2, [r0, #8]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r6, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r5\n\t"
        "	strh r2, [r0, #0xa]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r6, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r5\n\t"
        "	strh r2, [r0, #0xc]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r6, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r5\n\t"
        "	movs r2, #1\n\t"
        "	strh r2, [r0, #0xe]\n\t"
        "	movs r0, #4\n\t"
        "	ldrsh r1, [r6, r0]\n\t"
        "	lsls r0, r1, #2\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #3\n\t"
        "	adds r0, r0, r5\n\t"
        "	strh r2, [r0, #0x10]\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D87A4: .4byte gUnknown_3005B68\n\t"
        "_081D87A8: .4byte 0x00000241\n\t"
        "_081D87AC: .4byte 0x00000E09\n\t"
        "_081D87B0: .4byte VBlankCB_RayquazaScene + 1\n\t"
        "_081D87B4: .4byte Task_HandleRayChasesAway + 1\n\t"
        "_081D87B8: .4byte sub_081D88EC + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_HandleRayChasesAway(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	mov r7, r8\n\t"
        "	push {r7}\n\t"
        "	sub sp, #0xc\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r6, r0, #0x18\n\t"
        "	lsls r0, r6, #2\n\t"
        "	adds r0, r0, r6\n\t"
        "	lsls r7, r0, #3\n\t"
        "	ldr r0, _081D87E8\n\t"
        "	mov r8, r0\n\t"
        "	adds r4, r7, r0\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r5, [r4, r1]\n\t"
        "	cmp r5, #1\n\t"
        "	beq _081D881A\n\t"
        "	cmp r5, #1\n\t"
        "	bgt _081D87EC\n\t"
        "	cmp r5, #0\n\t"
        "	beq _081D87F6\n\t"
        "	b _081D88DC\n\t"
        "	.align 2, 0\n\t"
        "_081D87E8: .4byte gUnknown_3005B68\n\t"
        "_081D87EC:\n\t"
        "	cmp r5, #2\n\t"
        "	beq _081D885A\n\t"
        "	cmp r5, #3\n\t"
        "	beq _081D88C0\n\t"
        "	b _081D88DC\n\t"
        "_081D87F6:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #8\n\t"
        "	bne _081D8854\n\t"
        "	adds r0, r6, #0\n\t"
        "	bl sub_081D89E4\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	str r5, [sp]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	strh r5, [r4, #2]\n\t"
        "	b _081D886C\n\t"
        "_081D881A:\n\t"
        "	ldr r2, _081D884C\n\t"
        "	movs r0, #0xa\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r2, #0x1c\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, [r0]\n\t"
        "	ldr r0, _081D8850\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D88DC\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x40\n\t"
        "	bne _081D8854\n\t"
        "	adds r0, r6, #0\n\t"
        "	bl sub_081D8E10\n\t"
        "	adds r0, r6, #0\n\t"
        "	bl sub_081D8D14\n\t"
        "	b _081D8868\n\t"
        "	.align 2, 0\n\t"
        "_081D884C: .4byte gSprites\n\t"
        "_081D8850: .4byte sub_081D911C + 1\n\t"
        "_081D8854:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	b _081D88DC\n\t"
        "_081D885A:\n\t"
        "	ldrh r2, [r4, #2]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	movs r0, #0xe0\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D8874\n\t"
        "_081D8868:\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r4, #2]\n\t"
        "_081D886C:\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	b _081D88DC\n\t"
        "_081D8874:\n\t"
        "	adds r0, r2, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	movs r1, #2\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	movs r1, #0x90\n\t"
        "	bl __modsi3\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r4, r0, #0x10\n\t"
        "	cmp r4, #0\n\t"
        "	bne _081D88DC\n\t"
        "	ldr r0, _081D88B4\n\t"
        "	ldr r1, _081D88B8\n\t"
        "	str r1, [sp]\n\t"
        "	str r4, [sp, #4]\n\t"
        "	str r4, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	ldr r0, _081D88BC\n\t"
        "	str r4, [sp]\n\t"
        "	str r4, [sp, #4]\n\t"
        "	movs r1, #1\n\t"
        "	str r1, [sp, #8]\n\t"
        "	movs r1, #0\n\t"
        "	movs r2, #0x10\n\t"
        "	movs r3, #0\n\t"
        "	bl sub_080A250C\n\t"
        "	b _081D88DC\n\t"
        "	.align 2, 0\n\t"
        "_081D88B4: .4byte 0x0000FFFE\n\t"
        "_081D88B8: .4byte 0x0000FFFF\n\t"
        "_081D88BC: .4byte 0xFFFF0000\n\t"
        "_081D88C0:\n\t"
        "	movs r0, #1\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	movs r1, #0\n\t"
        "	str r1, [sp]\n\t"
        "	movs r1, #4\n\t"
        "	movs r2, #0\n\t"
        "	movs r3, #0x10\n\t"
        "	bl BeginNormalPaletteFade\n\t"
        "	mov r0, r8\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r7, r0\n\t"
        "	ldr r1, _081D88E8\n\t"
        "	str r1, [r0]\n\t"
        "_081D88DC:\n\t"
        "	add sp, #0xc\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D88E8: .4byte Task_RayChasesAwayEnd + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D88EC(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D896C\n\t"
        "	adds r4, r1, r0\n\t"
        "	ldrh r1, [r4]\n\t"
        "	movs r0, #0xf\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8960\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	adds r1, #0xe\n\t"
        "	lsls r1, r1, #8\n\t"
        "	movs r2, #0xf8\n\t"
        "	lsls r2, r2, #5\n\t"
        "	adds r0, r2, #0\n\t"
        "	ands r1, r0\n\t"
        "	ldrh r0, [r4, #4]\n\t"
        "	adds r0, #9\n\t"
        "	movs r2, #0xf\n\t"
        "	ands r0, r2\n\t"
        "	orrs r1, r0\n\t"
        "	movs r0, #0x52\n\t"
        "	bl SetGpuReg\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	ldrh r0, [r4, #6]\n\t"
        "	subs r1, r1, r0\n\t"
        "	strh r1, [r4, #2]\n\t"
        "	ldrh r0, [r4, #8]\n\t"
        "	ldrh r2, [r4, #4]\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x10\n\t"
        "	movs r0, #3\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	cmp r1, r0\n\t"
        "	beq _081D8944\n\t"
        "	cmp r1, #0\n\t"
        "	bne _081D894C\n\t"
        "_081D8944:\n\t"
        "	movs r1, #6\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	strh r0, [r4, #6]\n\t"
        "_081D894C:\n\t"
        "	movs r2, #4\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #3\n\t"
        "	beq _081D8958\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8960\n\t"
        "_081D8958:\n\t"
        "	movs r1, #8\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	strh r0, [r4, #8]\n\t"
        "_081D8960:\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D896C: .4byte gUnknown_3005B68\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void Task_RayChasesAwayEnd(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r6, r1, #3\n\t"
        "	ldr r7, _081D89CC\n\t"
        "	adds r4, r6, r7\n\t"
        "	ldr r0, _081D89D0\n\t"
        "	ldrb r1, [r0, #7]\n\t"
        "	movs r0, #0x80\n\t"
        "	ands r0, r1\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	cmp r5, #0\n\t"
        "	bne _081D89DC\n\t"
        "	bl StopMapMusic\n\t"
        "	movs r1, #2\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D89B4\n\t"
        "	movs r0, #0\n\t"
        "	bl SetVBlankCallback\n\t"
        "	bl sub_081D6240\n\t"
        "	bl ResetSpriteData\n\t"
        "	bl FreeAllSpritePalettes\n\t"
        "	ldrb r0, [r4, #4]\n\t"
        "	bl DestroyTask\n\t"
        "_081D89B4:\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmp r0, #0x20\n\t"
        "	bne _081D89D8\n\t"
        "	strh r5, [r4, #2]\n\t"
        "	adds r0, r7, #0\n\t"
        "	subs r0, #8\n\t"
        "	adds r0, r6, r0\n\t"
        "	ldr r1, _081D89D4\n\t"
        "	str r1, [r0]\n\t"
        "	b _081D89DC\n\t"
        "	.align 2, 0\n\t"
        "_081D89CC: .4byte gUnknown_3005B68\n\t"
        "_081D89D0: .4byte gPaletteFade\n\t"
        "_081D89D4: .4byte Task_SetNextAnim + 1\n\t"
        "_081D89D8:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "_081D89DC:\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D89E4(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	mov r7, sl\n\t"
        "	mov r6, sb\n\t"
        "	mov r5, r8\n\t"
        "	push {r5, r6, r7}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r4, r0, #2\n\t"
        "	adds r4, r4, r0\n\t"
        "	lsls r4, r4, #3\n\t"
        "	ldr r0, _081D8B94\n\t"
        "	adds r4, r4, r0\n\t"
        "	ldr r0, _081D8B98\n\t"
        "	movs r1, #0x40\n\t"
        "	movs r2, #0x78\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	ldr r1, _081D8B9C\n\t"
        "	mov sl, r1\n\t"
        "	adds r7, r0, r1\n\t"
        "	ldr r0, _081D8BA0\n\t"
        "	movs r1, #0x10\n\t"
        "	movs r2, #0x82\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7]\n\t"
        "	movs r2, #0x2e\n\t"
        "	rsbs r2, r2, #0\n\t"
        "	add r2, sl\n\t"
        "	mov r8, r2\n\t"
        "	movs r1, #6\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	movs r6, #0xd\n\t"
        "	rsbs r6, r6, #0\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	movs r2, #4\n\t"
        "	mov sb, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	ldr r5, _081D8BA4\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xa0\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	mov r1, sl\n\t"
        "	adds r7, r0, r1\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xc0\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7]\n\t"
        "	adds r0, r5, #0\n\t"
        "	movs r1, #0xe0\n\t"
        "	movs r2, #0x80\n\t"
        "	movs r3, #1\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7, #2]\n\t"
        "	movs r2, #8\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	movs r1, #0\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	movs r1, #2\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r7, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	add r0, r8\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	movs r2, #2\n\t"
        "	ldrsh r1, [r7, r2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	add r0, r8\n\t"
        "	movs r1, #2\n\t"
        "	bl StartSpriteAnim\n\t"
        "	ldr r0, _081D8BA8\n\t"
        "	movs r2, #0x41\n\t"
        "	rsbs r2, r2, #0\n\t"
        "	movs r1, #0x78\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r4, #0xa]\n\t"
        "	movs r0, #0xa\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	mov r1, sl\n\t"
        "	adds r7, r0, r1\n\t"
        "	ldr r0, _081D8BAC\n\t"
        "	movs r2, #0x71\n\t"
        "	rsbs r2, r2, #0\n\t"
        "	movs r1, #0x78\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7]\n\t"
        "	movs r2, #0xa\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	add r1, r8\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	adds r0, r6, #0\n\t"
        "	ands r0, r2\n\t"
        "	mov r2, sb\n\t"
        "	orrs r0, r2\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r7, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	add r0, r8\n\t"
        "	ldrb r1, [r0, #5]\n\t"
        "	ands r6, r1\n\t"
        "	orrs r6, r2\n\t"
        "	strb r6, [r0, #5]\n\t"
        "	pop {r3, r4, r5}\n\t"
        "	mov r8, r3\n\t"
        "	mov sb, r4\n\t"
        "	mov sl, r5\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8B94: .4byte gUnknown_3005B68\n\t"
        "_081D8B98: .4byte sSpriteTemplate_ChasesAway_Groudon\n\t"
        "_081D8B9C: .4byte gUnknown_20205DA\n\t"
        "_081D8BA0: .4byte sSpriteTemplate_ChasesAway_GroudonTail\n\t"
        "_081D8BA4: .4byte sSpriteTemplate_ChasesAway_Kyogre\n\t"
        "_081D8BA8: .4byte sSpriteTemplate_ChasesAway_Rayquaza\n\t"
        "_081D8BAC: .4byte sSpriteTemplate_ChasesAway_RayquazaTail\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8BB0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, lr}\n\t"
        "	mov r6, r8\n\t"
        "	push {r6}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r2, r0, #2\n\t"
        "	adds r2, r2, r0\n\t"
        "	lsls r2, r2, #3\n\t"
        "	ldr r0, _081D8C68\n\t"
        "	adds r2, r2, r0\n\t"
        "	ldr r3, _081D8C6C\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	movs r1, #0x1c\n\t"
        "	adds r1, r1, r3\n\t"
        "	mov r8, r1\n\t"
        "	add r0, r8\n\t"
        "	ldr r6, _081D8C70\n\t"
        "	str r6, [r0]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	movs r4, #0\n\t"
        "	strh r4, [r0, #0x36]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r4, [r0, #0x38]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	movs r5, #4\n\t"
        "	strh r5, [r0, #0x3a]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r4, [r0, #0x3c]\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	add r0, r8\n\t"
        "	str r6, [r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r4, [r0, #0x36]\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r4, [r0, #0x38]\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r5, [r0, #0x3a]\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	movs r1, #1\n\t"
        "	strh r1, [r0, #0x3c]\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8C68: .4byte gUnknown_3005B68\n\t"
        "_081D8C6C: .4byte gSprites\n\t"
        "_081D8C70: .4byte sub_081D8C74 + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8C74(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	adds r4, r0, #0\n\t"
        "	ldrh r1, [r4, #0x36]\n\t"
        "	movs r0, #7\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8D08\n\t"
        "	movs r1, #0x3c\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8CAC\n\t"
        "	ldrh r0, [r4, #0x20]\n\t"
        "	ldrh r3, [r4, #0x3a]\n\t"
        "	subs r0, r0, r3\n\t"
        "	strh r0, [r4, #0x20]\n\t"
        "	ldr r2, _081D8CA8\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, r1, r3\n\t"
        "	b _081D8CDA\n\t"
        "	.align 2, 0\n\t"
        "_081D8CA8: .4byte gSprites\n\t"
        "_081D8CAC:\n\t"
        "	ldrh r2, [r4, #0x3a]\n\t"
        "	ldrh r1, [r4, #0x20]\n\t"
        "	adds r0, r2, r1\n\t"
        "	strh r0, [r4, #0x20]\n\t"
        "	ldr r3, _081D8D00\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r2, r2, r1\n\t"
        "	strh r2, [r0, #0x20]\n\t"
        "	movs r2, #0x30\n\t"
        "	ldrsh r1, [r4, r2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	ldrh r1, [r4, #0x3a]\n\t"
        "	ldrh r2, [r0, #0x20]\n\t"
        "	adds r1, r1, r2\n\t"
        "_081D8CDA:\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "	ldrh r1, [r4, #0x38]\n\t"
        "	adds r1, #1\n\t"
        "	movs r2, #0\n\t"
        "	strh r1, [r4, #0x38]\n\t"
        "	ldrh r0, [r4, #0x3a]\n\t"
        "	subs r0, r0, r1\n\t"
        "	strh r0, [r4, #0x3a]\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x10\n\t"
        "	cmp r1, #3\n\t"
        "	bne _081D8D08\n\t"
        "	strh r2, [r4, #0x36]\n\t"
        "	strh r2, [r4, #0x38]\n\t"
        "	strh r2, [r4, #0x3a]\n\t"
        "	ldr r0, _081D8D04\n\t"
        "	str r0, [r4, #0x1c]\n\t"
        "	b _081D8D0E\n\t"
        "	.align 2, 0\n\t"
        "_081D8D00: .4byte gSprites\n\t"
        "_081D8D04: .4byte SpriteCallbackDummy + 1\n\t"
        "_081D8D08:\n\t"
        "	ldrh r0, [r4, #0x36]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #0x36]\n\t"
        "_081D8D0E:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8D14(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r2, r0, #2\n\t"
        "	adds r2, r2, r0\n\t"
        "	lsls r2, r2, #3\n\t"
        "	ldr r0, _081D8D50\n\t"
        "	adds r2, r2, r0\n\t"
        "	ldr r3, _081D8D54\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r1, r3, #0\n\t"
        "	adds r1, #0x1c\n\t"
        "	adds r0, r0, r1\n\t"
        "	ldr r1, _081D8D58\n\t"
        "	str r1, [r0]\n\t"
        "	movs r0, #6\n\t"
        "	ldrsh r1, [r2, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	movs r1, #1\n\t"
        "	bl StartSpriteAnim\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8D50: .4byte gUnknown_3005B68\n\t"
        "_081D8D54: .4byte gSprites\n\t"
        "_081D8D58: .4byte sub_081D8D5C + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8D5C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	adds r4, r0, #0\n\t"
        "	adds r0, #0x2b\n\t"
        "	ldrb r0, [r0]\n\t"
        "	cmp r0, #1\n\t"
        "	beq _081D8DC4\n\t"
        "	cmp r0, #1\n\t"
        "	bgt _081D8D72\n\t"
        "	cmp r0, #0\n\t"
        "	beq _081D8D7C\n\t"
        "	b _081D8E00\n\t"
        "_081D8D72:\n\t"
        "	cmp r0, #2\n\t"
        "	beq _081D8D7C\n\t"
        "	cmp r0, #3\n\t"
        "	beq _081D8DC4\n\t"
        "	b _081D8E00\n\t"
        "_081D8D7C:\n\t"
        "	adds r0, r4, #0\n\t"
        "	adds r0, #0x2c\n\t"
        "	ldrb r0, [r0]\n\t"
        "	lsls r0, r0, #0x1a\n\t"
        "	lsrs r0, r0, #0x1a\n\t"
        "	movs r1, #0xc\n\t"
        "	bl __umodsi3\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	ldr r2, _081D8DC0\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8DAC\n\t"
        "	ldrh r0, [r4, #0x20]\n\t"
        "	subs r0, #2\n\t"
        "	strh r0, [r4, #0x20]\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	subs r1, #2\n\t"
        "	strh r1, [r0, #0x20]\n\t"
        "_081D8DAC:\n\t"
        "	movs r1, #0x2e\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	adds r1, r1, r2\n\t"
        "	movs r0, #0\n\t"
        "	strh r0, [r1, #0x26]\n\t"
        "	b _081D8E00\n\t"
        "	.align 2, 0\n\t"
        "_081D8DC0: .4byte gSprites\n\t"
        "_081D8DC4:\n\t"
        "	ldr r2, _081D8E08\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r1, _081D8E0C\n\t"
        "	strh r1, [r0, #0x26]\n\t"
        "	adds r0, r4, #0\n\t"
        "	adds r0, #0x2c\n\t"
        "	ldrb r1, [r0]\n\t"
        "	lsls r1, r1, #0x1a\n\t"
        "	lsrs r1, r1, #0x1a\n\t"
        "	movs r0, #0xf\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8E00\n\t"
        "	ldrh r0, [r4, #0x22]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #0x22]\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r1, #1\n\t"
        "	strh r1, [r0, #0x22]\n\t"
        "_081D8E00:\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8E08: .4byte gSprites\n\t"
        "_081D8E0C: .4byte 0x0000FFFE\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8E10(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, lr}\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	lsls r1, r0, #2\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #3\n\t"
        "	ldr r0, _081D8E58\n\t"
        "	adds r1, r1, r0\n\t"
        "	movs r0, #8\n\t"
        "	ldrsh r1, [r1, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	ldr r2, _081D8E5C\n\t"
        "	adds r4, r0, r2\n\t"
        "	subs r2, #0x12\n\t"
        "	adds r0, r0, r2\n\t"
        "	ldr r3, _081D8E60\n\t"
        "	str r3, [r0]\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	str r3, [r0]\n\t"
        "	movs r0, #2\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	str r3, [r0]\n\t"
        "	pop {r4}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D8E58: .4byte gUnknown_3005B68\n\t"
        "_081D8E5C: .4byte gUnknown_20205DA\n\t"
        "_081D8E60: .4byte sub_081D8E64 + 1\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D8E64(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	mov r7, r8\n\t"
        "	push {r7}\n\t"
        "	adds r7, r0, #0\n\t"
        "	ldrh r1, [r7, #0x36]\n\t"
        "	movs r0, #3\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8E8C\n\t"
        "	ldrh r1, [r7, #0x24]\n\t"
        "	movs r2, #0x24\n\t"
        "	ldrsh r0, [r7, r2]\n\t"
        "	cmp r0, #1\n\t"
        "	bne _081D8E88\n\t"
        "	ldr r0, _081D8E84\n\t"
        "	b _081D8E8A\n\t"
        "	.align 2, 0\n\t"
        "_081D8E84: .4byte 0x0000FFFF\n\t"
        "_081D8E88:\n\t"
        "	movs r0, #1\n\t"
        "_081D8E8A:\n\t"
        "	strh r0, [r7, #0x24]\n\t"
        "_081D8E8C:\n\t"
        "	movs r1, #0x38\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	cmp r0, #0x80\n\t"
        "	bne _081D8F0A\n\t"
        "	ldr r2, _081D8F2C\n\t"
        "	mov r8, r2\n\t"
        "	mov r0, r8\n\t"
        "	movs r1, #0x98\n\t"
        "	movs r2, #0x84\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7, #0x3c]\n\t"
        "	ldr r5, _081D8F30\n\t"
        "	movs r1, #0x3c\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	lsls r1, r0, #4\n\t"
        "	adds r1, r1, r0\n\t"
        "	lsls r1, r1, #2\n\t"
        "	adds r1, r1, r5\n\t"
        "	ldrb r2, [r1, #5]\n\t"
        "	movs r4, #0xd\n\t"
        "	rsbs r4, r4, #0\n\t"
        "	adds r0, r4, #0\n\t"
        "	ands r0, r2\n\t"
        "	movs r6, #4\n\t"
        "	orrs r0, r6\n\t"
        "	strb r0, [r1, #5]\n\t"
        "	mov r0, r8\n\t"
        "	movs r1, #0xe0\n\t"
        "	movs r2, #0x84\n\t"
        "	movs r3, #0\n\t"
        "	bl CreateSprite\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	strh r0, [r7, #0x3c]\n\t"
        "	movs r2, #0x3c\n\t"
        "	ldrsh r1, [r7, r2]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	ldrb r1, [r0, #5]\n\t"
        "	ands r4, r1\n\t"
        "	orrs r4, r6\n\t"
        "	strb r4, [r0, #5]\n\t"
        "	movs r0, #0x3c\n\t"
        "	ldrsh r1, [r7, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	adds r0, #0x3f\n\t"
        "	ldrb r1, [r0]\n\t"
        "	movs r2, #1\n\t"
        "	orrs r1, r2\n\t"
        "	strb r1, [r0]\n\t"
        "	ldrh r0, [r7, #0x38]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r7, #0x38]\n\t"
        "_081D8F0A:\n\t"
        "	ldrh r1, [r7, #0x38]\n\t"
        "	movs r2, #0x38\n\t"
        "	ldrsh r0, [r7, r2]\n\t"
        "	cmp r0, #0x7f\n\t"
        "	ble _081D8F34\n\t"
        "	movs r1, #0x26\n\t"
        "	ldrsh r0, [r7, r1]\n\t"
        "	cmp r0, #0x20\n\t"
        "	beq _081D8F38\n\t"
        "	ldrh r0, [r7, #0x3a]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r7, #0x3a]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x14\n\t"
        "	strh r0, [r7, #0x26]\n\t"
        "	b _081D8F38\n\t"
        "	.align 2, 0\n\t"
        "_081D8F2C: .4byte sSpriteTemplate_ChasesAway_KyogreSplash\n\t"
        "_081D8F30: .4byte gSprites\n\t"
        "_081D8F34:\n\t"
        "	adds r0, r1, #1\n\t"
        "	strh r0, [r7, #0x38]\n\t"
        "_081D8F38:\n\t"
        "	ldrh r0, [r7, #0x36]\n\t"
        "	movs r1, #0x3f\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8F48\n\t"
        "	movs r0, #0xa5\n\t"
        "	bl PlaySE\n\t"
        "_081D8F48:\n\t"
        "	ldrh r0, [r7, #0x36]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r7, #0x36]\n\t"
        "	pop {r3}\n\t"
        "	mov r8, r3\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void SpriteCB_ChasesAway_Rayquaza(struct Sprite *sprite)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	adds r4, r0, #0\n\t"
        "	movs r0, #0x3c\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	cmp r1, #0x40\n\t"
        "	bgt _081D8FC8\n\t"
        "	ldrh r0, [r4, #0x26]\n\t"
        "	adds r0, #2\n\t"
        "	strh r0, [r4, #0x26]\n\t"
        "	ldr r5, _081D8FC0\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	ldrh r1, [r0, #0x26]\n\t"
        "	adds r1, #2\n\t"
        "	strh r1, [r0, #0x26]\n\t"
        "	movs r1, #0x3c\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0x40\n\t"
        "	beq _081D8F88\n\t"
        "	b _081D90DA\n\t"
        "_081D8F88:\n\t"
        "	movs r3, #0x30\n\t"
        "	rsbs r3, r3, #0\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #1\n\t"
        "	movs r2, #0\n\t"
        "	bl sub_081D91A4\n\t"
        "	movs r2, #5\n\t"
        "	strh r2, [r4, #0x36]\n\t"
        "	ldr r0, _081D8FC4\n\t"
        "	strh r0, [r4, #0x38]\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	movs r1, #3\n\t"
        "	strh r1, [r0, #0x36]\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	strh r2, [r0, #0x38]\n\t"
        "	b _081D90DA\n\t"
        "	.align 2, 0\n\t"
        "_081D8FC0: .4byte gSprites\n\t"
        "_081D8FC4: .4byte 0x0000FFFF\n\t"
        "_081D8FC8:\n\t"
        "	cmp r1, #0x6f\n\t"
        "	bgt _081D8FFA\n\t"
        "	adds r0, r4, #0\n\t"
        "	bl sub_081D911C\n\t"
        "	movs r1, #0x36\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D8FE0\n\t"
        "	movs r0, #0x68\n\t"
        "	bl PlaySE\n\t"
        "_081D8FE0:\n\t"
        "	movs r0, #0x36\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	movs r0, #3\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D90DA\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #2\n\t"
        "	movs r2, #0x30\n\t"
        "	movs r3, #0x10\n\t"
        "	bl sub_081D91A4\n\t"
        "	b _081D90DA\n\t"
        "_081D8FFA:\n\t"
        "	cmp r1, #0x70\n\t"
        "	bne _081D9028\n\t"
        "	ldr r2, _081D9024\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r1, #7\n\t"
        "	strh r1, [r0, #0x36]\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	movs r1, #3\n\t"
        "	strh r1, [r0, #0x38]\n\t"
        "	b _081D902E\n\t"
        "	.align 2, 0\n\t"
        "_081D9024: .4byte gSprites\n\t"
        "_081D9028:\n\t"
        "	ldr r0, _081D9038\n\t"
        "	cmp r1, r0\n\t"
        "	bgt _081D903C\n\t"
        "_081D902E:\n\t"
        "	adds r0, r4, #0\n\t"
        "	bl sub_081D911C\n\t"
        "	b _081D90DA\n\t"
        "	.align 2, 0\n\t"
        "_081D9038: .4byte SPECIAL_IsSelectedMonEgg\n\t"
        "_081D903C:\n\t"
        "	movs r0, #0xa4\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D9088\n\t"
        "	adds r0, r4, #0\n\t"
        "	bl sub_081D911C\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #3\n\t"
        "	movs r2, #0x30\n\t"
        "	movs r3, #0x10\n\t"
        "	bl sub_081D91A4\n\t"
        "	movs r3, #1\n\t"
        "	strh r3, [r4, #0x24]\n\t"
        "	ldr r2, _081D9080\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	movs r0, #0xcb\n\t"
        "	lsls r0, r0, #1\n\t"
        "	movs r1, #0\n\t"
        "	bl PlayCry1\n\t"
        "	ldr r0, _081D9084\n\t"
        "	movs r1, #0\n\t"
        "	bl CreateTask\n\t"
        "	b _081D90DA\n\t"
        "	.align 2, 0\n\t"
        "_081D9080: .4byte gSprites\n\t"
        "_081D9084: .4byte sub_081D91F0 + 1\n\t"
        "_081D9088:\n\t"
        "	movs r0, #0xb0\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	beq _081D90CC\n\t"
        "	adds r0, #0x18\n\t"
        "	cmp r1, r0\n\t"
        "	bne _081D90DA\n\t"
        "	movs r3, #0\n\t"
        "	strh r3, [r4, #0x24]\n\t"
        "	ldr r2, _081D90C4\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r3, [r0, #0x24]\n\t"
        "	adds r0, r4, #0\n\t"
        "	bl sub_081D911C\n\t"
        "	adds r0, r4, #0\n\t"
        "	movs r1, #2\n\t"
        "	movs r2, #0x30\n\t"
        "	movs r3, #0x10\n\t"
        "	bl sub_081D91A4\n\t"
        "	ldr r0, _081D90C8\n\t"
        "	str r0, [r4, #0x1c]\n\t"
        "	b _081D910E\n\t"
        "	.align 2, 0\n\t"
        "_081D90C4: .4byte gSprites\n\t"
        "_081D90C8: .4byte sub_081D911C + 1\n\t"
        "_081D90CC:\n\t"
        "	ldr r0, _081D9114\n\t"
        "	bl FindTaskIdByFunc\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r0, r0, #0x18\n\t"
        "	bl sub_081D8BB0\n\t"
        "_081D90DA:\n\t"
        "	ldrh r2, [r4, #0x3c]\n\t"
        "	movs r0, #0x3c\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	movs r0, #0xa4\n\t"
        "	lsls r0, r0, #1\n\t"
        "	cmp r1, r0\n\t"
        "	ble _081D9108\n\t"
        "	movs r0, #1\n\t"
        "	ands r0, r2\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D9108\n\t"
        "	movs r1, #0x24\n\t"
        "	ldrsh r2, [r4, r1]\n\t"
        "	rsbs r2, r2, #0\n\t"
        "	strh r2, [r4, #0x24]\n\t"
        "	ldr r3, _081D9118\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r3\n\t"
        "	strh r2, [r0, #0x24]\n\t"
        "_081D9108:\n\t"
        "	ldrh r0, [r4, #0x3c]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #0x3c]\n\t"
        "_081D910E:\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D9114: .4byte Task_HandleRayChasesAway + 1\n\t"
        "_081D9118: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D911C(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, r6, r7, lr}\n\t"
        "	adds r3, r0, #0\n\t"
        "	movs r0, #0x2e\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	ldr r5, _081D917C\n\t"
        "	adds r4, r0, r5\n\t"
        "	ldrh r1, [r3, #0x3a]\n\t"
        "	ldrh r0, [r4, #0x36]\n\t"
        "	ands r0, r1\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D9198\n\t"
        "	ldrh r2, [r3, #0x36]\n\t"
        "	ldrh r1, [r3, #0x26]\n\t"
        "	adds r0, r2, r1\n\t"
        "	strh r0, [r3, #0x26]\n\t"
        "	movs r7, #0x2e\n\t"
        "	ldrsh r1, [r3, r7]\n\t"
        "	lsls r0, r1, #4\n\t"
        "	adds r0, r0, r1\n\t"
        "	lsls r0, r0, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	ldrh r1, [r0, #0x26]\n\t"
        "	adds r2, r2, r1\n\t"
        "	strh r2, [r0, #0x26]\n\t"
        "	ldrh r0, [r3, #0x38]\n\t"
        "	ldrh r2, [r3, #0x36]\n\t"
        "	adds r0, r0, r2\n\t"
        "	strh r0, [r3, #0x36]\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r5, r0, #0x10\n\t"
        "	ldrh r6, [r4, #0x38]\n\t"
        "	movs r7, #0x38\n\t"
        "	ldrsh r2, [r4, r7]\n\t"
        "	cmp r5, r2\n\t"
        "	bge _081D9172\n\t"
        "	movs r0, #0x36\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	adds r0, r2, #0\n\t"
        "	cmn r1, r0\n\t"
        "	bgt _081D9198\n\t"
        "_081D9172:\n\t"
        "	cmp r5, r2\n\t"
        "	ble _081D9180\n\t"
        "	strh r6, [r3, #0x36]\n\t"
        "	b _081D9190\n\t"
        "	.align 2, 0\n\t"
        "_081D917C: .4byte gSprites\n\t"
        "_081D9180:\n\t"
        "	movs r0, #0x36\n\t"
        "	ldrsh r1, [r3, r0]\n\t"
        "	movs r2, #0x38\n\t"
        "	ldrsh r0, [r4, r2]\n\t"
        "	cmn r1, r0\n\t"
        "	bge _081D9190\n\t"
        "	rsbs r0, r6, #0\n\t"
        "	strh r0, [r3, #0x36]\n\t"
        "_081D9190:\n\t"
        "	movs r7, #0x38\n\t"
        "	ldrsh r0, [r3, r7]\n\t"
        "	rsbs r0, r0, #0\n\t"
        "	strh r0, [r3, #0x38]\n\t"
        "_081D9198:\n\t"
        "	ldrh r0, [r3, #0x3a]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r3, #0x3a]\n\t"
        "	pop {r4, r5, r6, r7}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D91A4(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	adds r5, r1, #0\n\t"
        "	lsls r5, r5, #0x18\n\t"
        "	lsrs r5, r5, #0x18\n\t"
        "	movs r4, #0x2e\n\t"
        "	ldrsh r1, [r0, r4]\n\t"
        "	lsls r4, r1, #4\n\t"
        "	adds r4, r4, r1\n\t"
        "	lsls r4, r4, #2\n\t"
        "	ldr r1, _081D91EC\n\t"
        "	adds r4, r4, r1\n\t"
        "	lsls r2, r2, #0x10\n\t"
        "	asrs r2, r2, #0x10\n\t"
        "	ldrh r1, [r0, #0x20]\n\t"
        "	adds r2, r2, r1\n\t"
        "	strh r2, [r4, #0x20]\n\t"
        "	lsls r3, r3, #0x10\n\t"
        "	asrs r3, r3, #0x10\n\t"
        "	ldrh r1, [r0, #0x22]\n\t"
        "	adds r3, r3, r1\n\t"
        "	strh r3, [r4, #0x22]\n\t"
        "	ldrh r1, [r0, #0x24]\n\t"
        "	strh r1, [r4, #0x24]\n\t"
        "	ldrh r1, [r0, #0x26]\n\t"
        "	strh r1, [r4, #0x26]\n\t"
        "	adds r1, r5, #0\n\t"
        "	bl StartSpriteAnim\n\t"
        "	adds r0, r4, #0\n\t"
        "	adds r1, r5, #0\n\t"
        "	bl StartSpriteAnim\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        "	.align 2, 0\n\t"
        "_081D91EC: .4byte gSprites\n\t"
        ".syntax divided\n\t"
    );
}

__attribute__((naked)) void sub_081D91F0(void)
{
    __asm__(".syntax unified\n\t"
        ".code 16\n\t"
        "	push {r4, r5, lr}\n\t"
        "	sub sp, #0x10\n\t"
        "	lsls r0, r0, #0x18\n\t"
        "	lsrs r5, r0, #0x18\n\t"
        "	lsls r0, r5, #2\n\t"
        "	adds r0, r0, r5\n\t"
        "	lsls r0, r0, #3\n\t"
        "	ldr r1, _081D9214\n\t"
        "	adds r4, r0, r1\n\t"
        "	movs r0, #0\n\t"
        "	ldrsh r1, [r4, r0]\n\t"
        "	cmp r1, #1\n\t"
        "	beq _081D9248\n\t"
        "	cmp r1, #1\n\t"
        "	bgt _081D9218\n\t"
        "	cmp r1, #0\n\t"
        "	beq _081D921E\n\t"
        "	b _081D92F0\n\t"
        "	.align 2, 0\n\t"
        "_081D9214: .4byte gUnknown_3005B68\n\t"
        "_081D9218:\n\t"
        "	cmp r1, #2\n\t"
        "	beq _081D92E0\n\t"
        "	b _081D92F0\n\t"
        "_081D921E:\n\t"
        "	movs r2, #0x80\n\t"
        "	lsls r2, r2, #7\n\t"
        "	movs r0, #0x40\n\t"
        "	str r0, [sp]\n\t"
        "	adds r0, #0xc0\n\t"
        "	str r0, [sp, #4]\n\t"
        "	str r0, [sp, #8]\n\t"
        "	str r1, [sp, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	adds r1, r2, #0\n\t"
        "	movs r3, #0x78\n\t"
        "	bl SetBgAffine\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #3\n\t"
        "	movs r0, #0\n\t"
        "	bl SetGpuRegBits\n\t"
        "	movs r0, #0x10\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	b _081D925E\n\t"
        "_081D9248:\n\t"
        "	movs r1, #0xa\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #8\n\t"
        "	bne _081D9256\n\t"
        "	movs r0, #0x12\n\t"
        "	bl PlaySE\n\t"
        "_081D9256:\n\t"
        "	movs r3, #4\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	cmp r0, #2\n\t"
        "	bne _081D9266\n\t"
        "_081D925E:\n\t"
        "	ldrh r0, [r4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4]\n\t"
        "	b _081D92F0\n\t"
        "_081D9266:\n\t"
        "	ldrh r5, [r4, #8]\n\t"
        "	ldrh r1, [r4, #2]\n\t"
        "	adds r0, r5, r1\n\t"
        "	strh r0, [r4, #2]\n\t"
        "	ldrh r0, [r4, #0xa]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #0xa]\n\t"
        "	movs r3, #6\n\t"
        "	ldrsh r0, [r4, r3]\n\t"
        "	movs r1, #3\n\t"
        "	bl __modsi3\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	cmp r0, #0\n\t"
        "	bne _081D9290\n\t"
        "	lsls r0, r5, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	cmp r0, #4\n\t"
        "	beq _081D9290\n\t"
        "	subs r0, r5, #2\n\t"
        "	strh r0, [r4, #8]\n\t"
        "_081D9290:\n\t"
        "	ldrh r0, [r4, #6]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #6]\n\t"
        "	movs r2, #0x80\n\t"
        "	lsls r2, r2, #7\n\t"
        "	movs r0, #0x40\n\t"
        "	str r0, [sp]\n\t"
        "	ldrh r0, [r4, #2]\n\t"
        "	movs r3, #0x80\n\t"
        "	lsls r3, r3, #1\n\t"
        "	adds r1, r3, #0\n\t"
        "	subs r0, r1, r0\n\t"
        "	lsls r0, r0, #0x10\n\t"
        "	asrs r0, r0, #0x10\n\t"
        "	str r0, [sp, #4]\n\t"
        "	ldrh r0, [r4, #2]\n\t"
        "	subs r1, r1, r0\n\t"
        "	lsls r1, r1, #0x10\n\t"
        "	asrs r1, r1, #0x10\n\t"
        "	str r1, [sp, #8]\n\t"
        "	movs r5, #0\n\t"
        "	str r5, [sp, #0xc]\n\t"
        "	movs r0, #2\n\t"
        "	adds r1, r2, #0\n\t"
        "	movs r3, #0x78\n\t"
        "	bl SetBgAffine\n\t"
        "	movs r1, #2\n\t"
        "	ldrsh r0, [r4, r1]\n\t"
        "	cmp r0, #0xff\n\t"
        "	ble _081D92F0\n\t"
        "	strh r5, [r4, #2]\n\t"
        "	strh r5, [r4, #6]\n\t"
        "	strh r5, [r4, #0xa]\n\t"
        "	movs r0, #0x10\n\t"
        "	strh r0, [r4, #8]\n\t"
        "	ldrh r0, [r4, #4]\n\t"
        "	adds r0, #1\n\t"
        "	strh r0, [r4, #4]\n\t"
        "	b _081D92F0\n\t"
        "_081D92E0:\n\t"
        "	movs r1, #0x80\n\t"
        "	lsls r1, r1, #3\n\t"
        "	movs r0, #0\n\t"
        "	bl ClearGpuRegBits\n\t"
        "	adds r0, r5, #0\n\t"
        "	bl DestroyTask\n\t"
        "_081D92F0:\n\t"
        "	add sp, #0x10\n\t"
        "	pop {r4, r5}\n\t"
        "	pop {r0}\n\t"
        "	bx r0\n\t"
        ".syntax divided\n\t"
    );
}
