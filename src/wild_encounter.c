...
static u8 ChooseWildMonLevel(const struct WildPokemon *wildPokemon)
{
    ...
    if (!GetMonData2(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
...
static u8 PickWildMonNature(void)
{
    ...
    if (!GetMonData2(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG)
        ...
        && Random() % 2 == 0)
    {
        return GetMonData2(&gPlayerParty[0], MON_DATA_PERSONALITY) % NUM_NATURES;
    }
...
static void CreateWildMon(u16 species, u8 level)
{
    ...
    if (checkCuteCharm
        && !GetMonData2(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG)
        && GetMonAbility(&gPlayerParty[0]) == ABILITY_CUTE_CHARM
        && Random() % 3 != 0)
    {
        u16 leadingMonSpecies = GetMonData2(&gPlayerParty[0], MON_DATA_SPECIES);
        u32 leadingMonPersonality = GetMonData2(&gPlayerParty[0], MON_DATA_PERSONALITY);
...
bool8 WildEncounterCheck(u32 encounterRate, bool8 ignoreAbility)
{
    ...
    if (!ignoreAbility && !GetMonData2(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
...
bool8 AllowWildCheckOnNewMetatile(void)
{
    if (Random() % 100 >= 60)
        return FALSE;
    else
        ...
        if (MetatileBehavior_IsLandWildEncounter(curMetatileBehavior) == TRUE)
        {
            ...
            else
            {
                ...
                return FALSE;
                 ...
                 || (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) && MetatileBehavior_IsBridge(curMetatileBehavior) == TRUE)) // JP: IsBridge
        {
...
bool8 IsWildLevelAllowedByRepel(u8 wildLevel)
{
    ...
    {
        if (GetMonData2(&gPlayerParty[i], MON_DATA_HP) && !GetMonData2(&gPlayerParty[i], MON_DATA_IS_EGG))
        {
            u8 ourLevel = GetMonData2(&gPlayerParty[i], MON_DATA_LEVEL);
...
static bool8 IsAbilityAllowingEncounter(u8 level)
{
    ...
    {
        u8 playerMonLevel = GetMonData2(&gPlayerParty[0], MON_DATA_LEVEL);
...
static void ApplyCleanseTagEncounterRateMod(u32 *encRate)
{
    if (GetMonData2(&gPlayerParty[0], MON_DATA_HELD_ITEM) == ITEM_CLEANSE_TAG)
        *encRate = *encRate * 2 / 3;
}
