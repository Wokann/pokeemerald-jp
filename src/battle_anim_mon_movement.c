...
static void AnimTask_ShakeAndSinkMon_Step(u8 taskId)
{
    ...
    gSprites[spriteId].y2 = (s8)(gTasks[taskId].data[9] >> 8);
...
// is mirrored for the opponent's Pok̩mon, and the vertical offset
...
static void AnimTask_WindUpLunge_Step1(u8 taskId)
{
    ...
    gSprites[spriteId].x2 = (s8)(gTasks[taskId].data[11] >> 8);
...
static void AnimTask_WindUpLunge_Step2(u8 taskId)
{
    ...
    else
    {
        ...
        gSprites[spriteId].x2 = (s8)(gTasks[taskId].data[12] >> 8) + (s8)(gTasks[taskId].data[11] >> 8);
