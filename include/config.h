...
// ---------------------------------------------------------------------------
// NONMATCHING: readable C implementations for functions that are currently
// kept as byte-exact inline asm (e.g. GenerateFontHalfRowLookupTable in
// src/text.c).
//
// - Not defined (default): the asm versions are compiled, keeping the output
//   ROM byte-identical to baserom_jp.gba (make compare must pass).
// - Defined: the C versions are compiled instead. They are much easier to
//   read and modify, but the generated bytes will differ from the original
//   ROM, so do not expect make compare to pass.
//
// To enable, either uncomment the line below, or build with:
//     make clean && make NONMATCHING=1
// ---------------------------------------------------------------------------
//#define NONMATCHING
